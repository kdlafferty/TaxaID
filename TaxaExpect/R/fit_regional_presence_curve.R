...
fit_regional_presence_curve <- function(
  ...
  if (!is.data.frame(presence_df) ||
    ...
    stop(
      "fit_regional_presence_curve: `presence_df` must be a data frame ",
  ...
  bin_table <- data.frame(
    ...
    jeffreys_ub_95 = c(
      .jeffreys_ub(as.integer(bin_x[keep]), as.integer(bin_n[keep])),
  ...
  if (n_present == 0L) {
    ...
    return(list(
      w_scale = NA_real_, d_half = NA_real_, fit = NULL,
  ...
  if (is.null(fit) || !fit$converged || stats::coef(fit)[2] >= 0) {
    ...
    if (is.null(fit) || !fit$converged || stats::coef(fit)[2] >= 0) {
      ...
      return(list(
        w_scale = NA_real_, d_half = NA_real_, fit = fit,
        identifiable = FALSE, bin_table = bin_table,
  ...
  list(
    w_scale = unname(w_scale), d_half = unname(d_half), fit = fit,
