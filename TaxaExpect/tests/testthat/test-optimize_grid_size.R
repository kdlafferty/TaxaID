...
test_that("stops if weights do not sum to 1", {
  expect_error(
    optimize_grid_size(.make_dense_obs(),
      n_covariates = 2,
...
test_that("always returns a list with four named elements", {
  out <- optimize_grid_size(.make_dense_obs(),
    n_covariates = 2,
    min_distinct_locs = 3L,
    min_s_threshold = .loose$min_s_threshold,
    min_N_threshold = .loose$min_N_threshold,
...
test_that("optimal path: summary_table has required columns", {
  ...
  expected_cols <- c(
    "grid_size", "suitability_score", "n_distinct_locs",
...
test_that("fallback C: triggered when data are too sparse for any multi-cell grid", {
  expect_warning(
    out <- optimize_grid_size(.make_single_point_obs(),
      n_covariates = 2,
...
test_that(".score_one_resolution returns a one-row tibble with correct columns", {
  ...
  out <- TaxaExpect:::.score_one_resolution(
    ...
    thresholds = list(
      min_s_threshold = 1L, min_N_threshold = 1L,
  ...
  expected_cols <- c(
    "grid_size", "n_samples", "n_distinct_locs",
...
test_that("habitat_col = NULL runs without a habitat column and returns a valid grid", {
  ...
  out <- optimize_grid_size(
    df,
    n_covariates = 2, habitat_col = NULL,
    min_s_threshold = .loose$min_s_threshold,
    min_N_threshold = .loose$min_N_threshold,
    min_distinct_locs = 3L,
    min_grid = .loose$min_grid,
