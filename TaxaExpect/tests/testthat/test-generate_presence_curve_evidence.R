...
test_that("k stretches the bandwidth (listed-invader lift, log-space halfway)", {
  ev1 <- generate_presence_curve_evidence("A a", w_scale = 0.05,
                                          distance_km = c("A a" = 300))
  ev2 <- generate_presence_curve_evidence("A a", w_scale = 0.05,
...
test_that("w_scale is required and validation fires", {
  ...
  expect_error(generate_presence_curve_evidence("A a", w_scale = 0.05,
                                                distance_km = c(1, 2)),
               "same length")
})
...
test_that("user-specified evidence carries provenance and discloses high weights", {
  expect_message(
    ev <- generate_user_specified_evidence(c("Gymnocephalus cernua" = 0.3,
                                             "Alosa alosa" = 0.02)),
    "dilution threshold")
...
test_that("generator output flows through curve-pricing applier end to end", {
  occ <- data.frame(taxon_name = c("A", "A", "A", "B", "C"),
  ...
  out <- apply_undetected_evidence(kp$priors, kp, ev, grid_id = "s",
