...
.sens_pool <- function() {
  ...
  common <- do.call(rbind, lapply(1:6, function(i) data.frame(
  ...
  rare <- do.call(rbind, lapply(1:12, function(i) data.frame(
    taxon_name = sprintf("Rare_%02d", i),
    decimalLatitude = 34 + (i %% 4) * 0.35,
    decimalLongitude = -119 + (i %/% 4) * 0.35,
    main_habitat = "Marine", sampling_group = "fish",
    stringsAsFactors = FALSE)))
  bird <- do.call(rbind, lapply(1:10, function(i) data.frame(
...
test_that("lambda_grid sweeps bandwidth as well as radius", {
  ...
  s <- suppressWarnings(kernel_budget_sensitivity(
    fit, occ, support_weight_grid = exp(-c(2, 3)), lambda_grid = c(10, 25, 50)))
...
test_that("input validation is explicit", {
  ...
  expect_error(kernel_budget_sensitivity(fit, occ, support_weight_grid = c(0, 0.5)),
               "\\(0, 1\\]")
  expect_error(kernel_budget_sensitivity(fit, occ, support_weight_grid = 2),
               "\\(0, 1\\]")
  expect_error(kernel_budget_sensitivity(fit, occ, lambda_grid = -5),
               "positive numerics")
})
