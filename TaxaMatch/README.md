...
The main input to TaxaID is a table where each row pairs an observation
(sequence, image, or sound recording) with a candidate taxon from a
...
TaxaMatch can also produce match tables from raw sequence data via NCBI
(National Center for Biotechnology Information, U.S. National Library of
Medicine, National Institutes of Health, Bethesda, Maryland) BLAST
(Basic Local Alignment Search Tool; Altschul et al. 1990), though most
users will start from an existing bioinformatics pipeline.
...
`animl` (Swanson and Tobler; Conservation Technology Lab, San Diego Zoo
...
`TaxaAssign::assign_taxa_llm()` as `unreferenced_taxa` (the LLM-shortcut
pipeline's own parameter for exactly this shape).

### Confirmed vs. Unconfirmed Assignments

Animl output typically contains two classes of predictions: confident
...
be cautious: hard exclusion on an imperfect quality proxy risks
systematically dropping whole taxa rather than just trimming noise.
Check whether the excluded records cluster in particular species before
trusting the result.
...
image. If images have GPS EXIF metadata, location is read automatically;
otherwise supply `lat`, `lng`, and `observed_on`. Scores are in
iNaturalist's 0--100 softmax scale (do not rescale). Requires an
...
covers general wildlife globally. Use `read_inaturalist_cv_output()` on
saved JSON response files:
...
TaxaLikely without access to the underlying softmax scores. A web
interface is available; programmatic access to model weights is
...
A mislabeled reference in the underlying NCBI database produces
confident wrong assignments that propagate to every query matching it.
...
``` r
# 1. Free, zero-NCBI-cost: does an independent conspecific already in your
#    own reference set corroborate each accession's label?
local_corr <- corroborate_references_locally(seq_matrix, reference_df)
table(local_corr$local_tier)   # singleton / same_batch_only / disagree / corroborated

# 2. BLAST against a broad, independent database -- only for accessions the
#    free check couldn't resolve.
ref_eval <- evaluate_reference_accessions(
  unique(reference_df$composite_id),
...
# 3. Combine both signals into one keep/remove verdict per accession.
...
# 4. Exclude before training a likelihood model.
...
framework, and `review_flagged_accessions()` (below) for an optional LLM
second look at ambiguous cases before removing anything.

`evaluate_reference_accessions()` BLASTs each accession's own deposited
sequence against a broad, unrestricted NCBI database, keeps hits that
are genuinely independent (not the same submission batch), and asks
...
If instead you're starting from a locally downloaded reference FASTA
(e.g. fetched via `TaxaLikely::fetch_ncbi_reference_sequences()` or
...
For a deeper dive on one specific flagged accession,
`investigate_flagged_accession()` (singular) /
`investigate_flagged_accessions()` (batch) re-BLAST against the
accession's own listed species and its top disagreeing taxon
specifically, with cached results. `check_marker_mismatch()` is a cheap
pre-check: does the record's own annotated `/gene`/`/product` qualifier
...
`{recording}_{start}-{end}` for acoustics) links posteriors back to the
original media files for review.
