...
# =============================================================================
# .is_trusted_pasta_url() / .download_data_table() -- SSRF host allowlist
# =============================================================================
# data_url is read from third-party-authored EML metadata (see
# .standardize_dataone_metadata()), not generated by this package -- these
# tests confirm requests are refused for any host outside the known
# PASTA/EDI download hosts, closing the SSRF surface without a live network
# call (the guard runs before .download_data_table() ever calls httr2).

test_that(".is_trusted_pasta_url() accepts known PASTA/EDI download hosts", {
  expect_true(TaxaFetch:::.is_trusted_pasta_url(
    "https://pasta.lternet.edu/package/data/eml/edi/1290/9/abc123"
  ))
  expect_true(TaxaFetch:::.is_trusted_pasta_url(
    "https://pasta.edirepository.org/package/data/eml/edi/1/1/abc"
  ))
})

test_that(".is_trusted_pasta_url() rejects hosts outside the allowlist", {
  expect_false(TaxaFetch:::.is_trusted_pasta_url("http://169.254.169.254/latest/meta-data"))
  expect_false(TaxaFetch:::.is_trusted_pasta_url("https://evil.example.com/data.csv"))
  expect_false(TaxaFetch:::.is_trusted_pasta_url("http://localhost:8080/admin"))
  expect_false(TaxaFetch:::.is_trusted_pasta_url("ftp://pasta.lternet.edu/data.csv"))
})

test_that(".is_trusted_pasta_url() rejects http (non-https) even for a trusted host", {
  expect_false(TaxaFetch:::.is_trusted_pasta_url(
    "http://pasta.lternet.edu/package/data/eml/edi/1290/9/abc123"
  ))
})

test_that(".is_trusted_pasta_url() handles unparseable input without erroring", {
  expect_false(TaxaFetch:::.is_trusted_pasta_url("not a url"))
  expect_false(TaxaFetch:::.is_trusted_pasta_url(""))
})

test_that(".download_data_table() refuses an untrusted data_url without a network call", {
  expect_message(
    result <- TaxaFetch:::.download_data_table("https://evil.example.com/data.csv"),
    "not a trusted PASTA/EDI host"
  )
  expect_null(result)
})
