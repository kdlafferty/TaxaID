...
test_that("returns a tibble with the correct nine columns", {
  ...
  expect_named(out, c("taxon_name", "taxon_id", "matched_name", "inat_kingdom",
...
test_that("captive and quality_grade filters are recorded on output", {
  ...
  out <- fetch_inat_occurrences("Calidris mauri", lat = 34.1, lng = -119.1,
...
test_that("verbose emits progress messages", {
  ...
  expect_message(
    fetch_inat_occurrences(c("Foo bar", "Baz qux"), lat = 0, lng = 0,
