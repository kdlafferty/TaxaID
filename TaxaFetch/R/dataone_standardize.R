...
#' GBIF itself. If you combine this output with GBIF occurrences (e.g. via
#' \code{\link{get_gbif_occurrences}}), pass both to
...
#' across sources -- see that function's Details for the full rationale.
#'
...
#' Trusted PASTA/EDI hosts for data-entity downloads
#'
#' \code{data_url} values are read from third-party-authored EML metadata
#' (\code{.standardize_dataone_metadata()}), not generated by this package --
#' an EML record's \code{<online><url>} entry can in principle point anywhere.
#' Verified directly against a real PASTA record (edi.1290.9) that every
#' \code{dataTable}/\code{otherEntity} download URL (\code{function="download"})
#' resolves to \code{pasta.lternet.edu}; \code{pasta.edirepository.org} is
#' EDI's own alternate system host for the same infrastructure. Restricting
#' requests to these hosts closes the SSRF surface (a malicious/malformed
#' \code{data_url} pointing at an internal address) without breaking any
#' legitimate PASTA download.
#'
#' @noRd
.pasta_trusted_hosts <- c("pasta.lternet.edu", "pasta.edirepository.org")

#' Check whether a URL's scheme+host is a trusted PASTA/EDI download host
#' @noRd
.is_trusted_pasta_url <- function(url) {
  ...
  identical(parsed$scheme, "https") &&
    tolower(parsed$hostname) %in% .pasta_trusted_hosts
}

#' Download a data table from an entity URL
#'
#' Detects CSV vs TSV from first-line delimiter counts.
#' Returns NULL with a message on HTTP or parse failure. Refuses to request
#' any \code{data_url} whose host is not a trusted PASTA/EDI host (see
#' \code{.is_trusted_pasta_url()}) -- \code{data_url} comes from third-party
#' EML metadata, not this package, and is not a safe target for an
