...
| `run_review_fixes_fast_check.R` | **Added 2026-09-13.** Unlike the four scripts above (which regression-check that fixtures built *before* a code change still reproduce -- proving nothing broke), this one deliberately EXERCISES five same-day ecosystem-review fixes against real data in explicit before/after arms and prints the difference, so it proves the fixes actually do something, not just that they don't crash. Stages 0-1 reproduce `run_ptcon18s_fast_smoketest.R`'s own real pipeline verbatim through `compute_posterior()` (including that script's one live ~10-name backbone call inside `join_priors()`). **Arm A** (`TaxaAssign::compute_group_priors(allowed_branches=)`): builds group priors with `allowed_branches = NULL` (old, branch-blind) vs. the new default -- on this fixture the fix collapses genus-level group-support rows 199 -> 3 and family-level 130 -> 3 (the 238 `resident_undetected` evidence-only rows can no longer support a group on their own), moving `consensus_has_occurrence_record` for 2/194 observations. **Arm B** (`posterior_consensus(downrank_requires_candidate=)` + the `species_reference` branch-filter change): a real, honest null result on this small fixture -- every `resident_undetected` row here is an anonymous genus/family-only placeholder with `taxon_name = NA`, so it was never eligible to BE a `species_reference` row under either the old or new construction; the script says so explicitly rather than papering over it. Only 1 downranking event occurs across 194 observations on this fixture and it's already legitimate (0 "outside" rows) in all 4 of the `species_reference` x `downrank_requires_candidate` combinations tested. **Arm C** (`TaxaAssign::combine_multisite_priors()`'s presence-mixture guard) is SEMI-SYNTHETIC and loudly labelled as such: this fixture predates curve pricing at this site and carries no `prior_mix_*` columns, so 3 real candidate rows are duplicated across two synthetic `grid_id`s with differing (2 rows) or identical (1 row, the control) presence-mixture pricing. Confirms: the mismatched-mixture warning fires naming the candidates, `prior_mix_*` are blanked to `NA` on the combined row while `prior_alpha`/`prior_beta` stay finite and positive, and the identical-mixture control passes through with its mixture preserved and no warning. **Arm D** (`TaxaTools::assign_sampling_group()`) runs against the real, full `PtCon18SSchulte_occurrences_clean.rds` (repo root, 2,185,193 rows, read-only) -- 0 ungrouped, matching `TaxaTools/CLAUDE.md`'s own regression number for this exact checkpoint. **Arm E** (`TaxaLikely::calibrate_query_noise()`'s bimodal-H1 diagnostic) runs the real trained model against real match data for PtCon 18S (the only one of the three named sites with a real `taxaexpect_priors` fixture in this directory to feed `identify_confident_observations()`) -- on this 200-observation fixture only 1 confident observation is found (need >= 30), so the bimodality check never gets to run at all; GreatLakes and PtCon 12S are SKIPPED outright (loudly, with a stated reason) since neither has a `taxaexpect_priors` fixture here and fabricating one would defeat the point. **Runs in well under a minute** (dominated by Stage 0-1's real `compute_posterior()` call and Arm D's full-checkpoint classification pass). |
