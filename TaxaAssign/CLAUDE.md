...
# naming. Investigation found a different shape of problem, not a true duplicate: this
# function's own roxygen already self-described as "a fast, LLM-first alternative to
# TaxaLikely::audit_barcode_coverage()" -- it does the identical reference-coverage-auditing
# job TaxaLikely already owns (see TaxaLikely/CLAUDE.md's own top note for the full
# investigation record, including the real-usage search across every real production
# workflow, `eDNA/`, `Stats and Data/`, and the broader `Rscripts/` tree). A thin
# `.Deprecated()` forwarding wrapper (`TaxaLikely::suggest_unreferenced_species(...)`, real
# callers get a working call with a deprecation warning rather than a hard failure) remains
# in `R/suggest_unreferenced_species.R`; `rentrez` dropped from Imports (no longer used by
# anything in this package once the function moved out -- confirmed via grep, closed a real
# `devtools::check()` NOTE).
#
# `run_llm_pipeline()`'s own internal call site (previously the bare, unqualified name,
# resolving to this package's own now-deprecated export) updated to
# `TaxaLikely::suggest_unreferenced_species()` directly, guarded by
# `requireNamespace("TaxaLikely", quietly = TRUE)` since TaxaLikely is only in this
# package's Suggests -- otherwise the package's own default `detect_unreferenced = TRUE`
# path would fire a deprecation warning on every single real call, found live via this
# session's own `devtools::test()` run (one new warning at `test-run_pipelines.R:103`,
# `'suggest_unreferenced_species' is deprecated`, gone after the fix). Both real in-package
# demo scripts (`inst/demo_for_JV.R`, `inst/TaxaAssign_llm_workflow.R`) updated to the new
# namespace too, even though the deprecated wrapper would have kept them working.
#
# Per the user's explicit request ("the readme needs to be much clearer that this is an LLM
# shortcut to be used in place of the modeled effort in TaxaLikely"), `README.md` and the
# roxygen for `assign_taxa_llm()`/`run_llm_pipeline()` rewritten: the "Two workflows"
# section now states plainly that Full Bayesian is the recommended pathway every real
# production workflow uses exclusively, and LLM-shortcut is an approximate *stand-in* for
# it (not a parallel, equally-supported pipeline) -- exponentially-weighted scores standing
# in for TaxaLikely's trained likelihood model, an LLM's biogeographic judgment standing in
# for TaxaExpect's occurrence-modelled prior. Quick Start reordered so Full Bayesian leads.
# `assign_taxa_llm()` gained a new `@section Purpose` making the same point at the function
# level (matching this ecosystem's own `score_consensus()` "@section Purpose" precedent for
# clarifying a function's actual intended role rather than leaving it to be inferred).
#
# Two pre-existing, unrelated roxygen bugs found and fixed in TaxaLikely while relocating
# this function's cross-references (not this package's own bug, but affecting how a
# TaxaAssign caller would have been misdirected) -- see TaxaLikely/CLAUDE.md's own note.
#
# `devtools::test()` 675/0 (55 pre-existing/expected warnings, down from 56 -- the
# deprecation-warning fix above), `devtools::check()` 0/0/0 (the pre-existing `rentrez`
# Imports NOTE is now gone too), reinstalled. See `TaxaID/CLAUDE.md`'s own 2026-09-08 top
# session note for the full ecosystem-level record.
# Previous update, 2026-09-08 (Sonnet 5 -- `run_llm_pipeline()` lost its `reference_errors`
# param and `run_bayesian_pipeline()` lost its automatic `model_params$reference_errors` ->
# `TaxaLikely::remove_flagged_references()` filtering step, both as a consequence of
# retiring `flag_reference_errors()`/`remove_flagged_references()` in TaxaLikely (see that
# package's own top session note and `TaxaID/CLAUDE.md`'s for the full ecosystem-level
# record). Found while mapping the full blast radius of that retirement -- these were two
# real, previously-unaccounted-for consumers of the mechanism, each removing rows from the
# QUERY match object (not the training reference set) whenever a trained model happened to
# carry flagged accessions. Confirmed via grep across all real external production
# workflows (GreatLakes + PtConception + Mugu) that NEITHER wrapper is actually called
# anywhere in real production use -- production workflows orchestrate the individual
# TaxaLikely/TaxaMatch/TaxaAssign functions directly rather than through these high-level
# wrappers -- so this was low-risk to remove outright rather than needing a replacement
# mechanism. Callers wanting the equivalent behavior should screen `match_df` themselves via
# `TaxaMatch::flag_incongruent_references()`/`remove_incongruent_references()` before
# calling either pipeline wrapper. `vignettes/taxaid-ecosystem.Rmd`'s
# `flag_reference_errors()` example call updated to a comment pointing at the new pattern.
# `devtools::test()`/`check()` re-verified clean after the removal.
# Previous update, 2026-09-06 (Sonnet 5 -- REAL, user-caught bug in generate_report()'s
...
| `suggest_unreferenced_species()` | **Moved to `TaxaLikely::suggest_unreferenced_species()` (2026-09-08, package-placement fix -- it's a reference-coverage-auditing function, the same job `TaxaLikely::audit_barcode_coverage()` does, so it belongs next to that function). This row's description previously described the full original implementation (LLM-first unreferenced species detection: plausible species per genus → reference-check → unreferenced vector; optional family expansion) -- stale as of the move; corrected 2026-09-09 during the ecosystem-wide accuracy pass.** `R/suggest_unreferenced_species.R` here is now only a thin, deprecated (`.Deprecated()`) forwarding wrapper -- every argument passed unchanged to `TaxaLikely::suggest_unreferenced_species()`, which is where the real algorithm/docs now live. Still exported (so an un-updated caller keeps working, with a warning), still technically "Complete," not "Archived." | Complete (deprecated forwarding wrapper) | R/suggest_unreferenced_species.R |
...
| `run_bayesian_pipeline()` | High-level wrapper: TaxaLikely likelihoods + TaxaExpect priors → full Bayesian workflow (~10 calls → 1). Auto-resolves site habitat. Stage 1b: three-tier H2 phantom suppression via GBIF genus census (suppress complete, rename singleton-missing, keep incomplete). GBIF species list fed to `audit_barcode_coverage(species_list=)`. **2026-09-08**: no longer auto-filters `match_df` against `model_params$reference_errors` -- that slot doesn't exist any more; screen `match_df` via TaxaMatch before calling this function (see `NAME_CHANGE_HISTORY.md`). | Complete | R/run_bayesian_pipeline.R |
| `run_llm_pipeline()` | High-level wrapper: LLM-shortcut workflow (~7 calls → 1); optional auto-context + unreferenced detection + report. **2026-09-08**: lost its `reference_errors` param -- screen `match_df` via TaxaMatch before calling this function (see `NAME_CHANGE_HISTORY.md`). | Complete | R/run_llm_pipeline.R |
