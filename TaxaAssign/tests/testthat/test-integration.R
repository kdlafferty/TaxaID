...
.make_match_df <- function() {
  tibble(
    ...
    score           = c(99.5, 95.2, 88.0, 97.1, 96.8, 80.0),
...
test_that("TaxaLikely likelihood columns feed into compute_posterior", {
  ...
  lik <- tibble(
    ...
    likelihood_point_est = c(0.85, 0.30, 0.15),
    likelihood_mean      = c(0.83, 0.31, 0.15),
    likelihood_sd        = c(0.05, 0.04, 0.03),
...
test_that("Full posterior pipeline: compute -> consensus -> empirical Bayes -> final consensus", {
  ...
  input <- bind_rows(
    tibble(
      ...
      likelihood_point_est = c(0.9, 0.1),
      likelihood_mean      = c(0.9, 0.1),
      likelihood_sd        = c(0.02, 0.02),
    ...
    tibble(
      ...
      taxon_name_rank      = "species",
      hypothesis_type      = "specific_candidate",
      likelihood_point_est = c(0.5, 0.5),
      likelihood_mean      = c(0.5, 0.5),
...
test_that("expand_unreferenced output feeds into compute_posterior and score_consensus", {
  lik <- data.frame(
    ...
    likelihood_point_est = c(0.90, 0.30),
    likelihood_mean      = c(0.90, 0.30),
    likelihood_sd        = c(0.05, 0.05),
