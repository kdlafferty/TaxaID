...
.mock_llm_result <- function() {
  df <- data.frame(
    ...
    taxon_name          = rep(c("Gobius niger", "Pomatoschistus minutus", "unknown_species"), 3),
    taxon_name_rank     = rep(c("species", "species", "species"), 3),
    hypothesis_type     = rep(c("specific_candidate", "specific_candidate", "unknown_species"), 3),
    ...
    likelihood_point_est = rep(c(0.6, 0.3, 0.1), 3),
    likelihood_mean      = rep(c(0.6, 0.3, 0.1), 3),
    likelihood_sd        = rep(c(0.05, 0.03, 0.01), 3),
