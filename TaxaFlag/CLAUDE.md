...
# hardest rows. That is not truncation, so status stays "complete" and the halving retry
# never fired; the taxon was NA-filled and, because every workflow's export chain tests
# != "unlikely" (which DISCARDS NA), the observation left the species list without a word.
# Same class of loss as the 2026-09-04 grass carp.
#
# (1) NEW re-ask: .parse_review_response() returns attr(, "missing_taxa");
# .review_batch_with_retry() re-asks for exactly those, up to max_retries, at the
# TERMINAL path of every batch -- so it also covers the leaves of the halving retry and
# the size-1 batch, where halving cannot fire at all (nrow > 1L). Stops early when a
# re-ask recovers nothing; never retries a hard llm_fn error. max_retries = 0 still means
# exactly one call per batch. .rbind_reviews() unions the residue across the recursion,
# since rbind() drops attributes.
# (2) SECOND DEFECT, not in the prompt: the NA row was being CACHED, so every later run
# was a HIT on a row of NAs and the omission was PERMANENT -- re-running could not
# recover it. An unreviewed taxon is now never written, and a cached entry holding no
# verdict is treated as a MISS, so existing caches heal themselves.
# (3) LOUD residue: attr(result, "unreviewed_taxa") + attr(, "n_unreviewed_rows"), always
# present (fetch_ncbi_reference_sequences()'s count_failures pattern), plus new
# on_unreviewed = c("warn","error","ignore"). Package default "warn" (non-breaking);
# every production workflow passes "error" (user decision -- the export filters stay
# != "unlikely" and a run with residue aborts instead of exporting short).
# (4) The prompt's "second bug" (unsorted slash label splits the review) is NOT A BUG and
# must not be "fixed": .make_slash_name() is posterior-ordered BY DESIGN (primary_taxon
# reads the first element), and review_assignments() has joined on the SORTED candidate
# set since 2026-09-04. The real data proves it -- the 20 affected PtCon rows carry 4
# display labels but only 3 CANONICAL SETS; both Scorpaenichthys orderings share one
# review, which the model omitted once. New regression test pins the order-invariance.
#
# VERIFIED LIVE on the real data the prompt names: the 20 unreviewed rows of
# PtConMifishSchulte_reviewed.rds re-run through the fixed function = 3 candidate sets,
# 1 LLM call, all 20 recovered (n_unreviewed_rows 20 -> 0), all three units passing the
# export filters (habitat likely/likely/possible, geographic possible/likely/likely,
# contamination low). So the loss was real and entirely recoverable: 20 observations of
# three plausible local fishes. That run did not itself exercise the re-ask (the model
# answered all three this time -- omission is stochastic), which is why the re-ask, the
# no-progress stop, the cache behaviour and on_unreviewed are pinned by deterministic
# unit tests instead. devtools::test() 530/0 (was 495), check() 0/0/0, reinstalled.
# Previous update, 2026-09-13, evening (Sonnet 5): ecosystem review Section L (G-C2) -- NEW
...
| `taxaflag_clear_cache()` | `R/taxaflag_clear_cache.R` | Written (2026-09-04) | **2026-09-09 doc gap (added during the ecosystem-wide accuracy pass; exported but missing from this table since it shipped).** Reports/clears `review_assignments(cache_dir=)`'s per-reviewed-taxon `.rds` cache -- same `cache_dir`/`older_than_days`/`dry_run` interface and shared `TaxaTools::list_cache_files()`/`report_and_clear_cache()` engine as `TaxaFetch::taxafetch_clear_cache()`/`TaxaLikely::taxalikely_clear_cache()`. |
...
**2026-09-09:** re-examined during an ecosystem-wide redundant/abandoned-function sweep
(zero callers confirmed to still hold, including in the one camera-trap workflow that
exists anywhere in the ecosystem, `TaxaAssign/inst/workflows/camera_trap_posterior_
workflow.R`, which does not call it either). **Explicit decision: keep, as documented,
available-but-unadopted infrastructure for camera-trap/timestamped-detection data** --
not a gap to close, same status as `build_review_covariates()`'s own already-documented
"kept anyway" verdict in `inst/taxaflag_review_response.md`. No code change.
