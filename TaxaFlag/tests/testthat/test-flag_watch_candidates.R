...
.make_consensus <- function() {
  data.frame(
    ...
    primary_taxon  = c("Perca flavescens", "Perca flavescens",
...
.make_match <- function() {
  data.frame(
    ...
    score_original = c(99.5, 97.0,   # A1: winner outscores watch -> no flag
                       98.0, 98.0,   # A2: watch TIES winner -> flag
                       99.0, 96.0,   # A3: winner IS the watch species -> no flag
                       99.0, 99.4),  # A4: watch outscores winner -> flag
    stringsAsFactors = FALSE
  )
}
...
test_that("winner without its own match row falls back to best non-watch score", {
  cons <- data.frame(observation_id = "B1", primary_taxon = "Cottus bairdii",
                     stringsAsFactors = FALSE)  # not in match rows
  m <- data.frame(observation_id = c("B1", "B1"),
                  taxon_name = c("Perca flavescens", "Sander lucioperca"),
...
test_that("no watch candidates and unmatched observations get FALSE with NA detail", {
  cons <- data.frame(observation_id = c("C1", "C2"), primary_taxon = "Perca flavescens",
                     stringsAsFactors = FALSE)
  m <- data.frame(observation_id = "C1", taxon_name = "Perca flavescens",
