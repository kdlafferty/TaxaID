...
# sequences() gained a Step 0 that serves every cached taxon BEFORE the count loop, so a
# cached taxon issues no count query at all and cannot be deleted by a transient NCBI
# failure (the 2026-09-14 incident: 7 genera lost, 78 sequences, 17 species -- every one
# of them had valid cached metadata on disk). Real 12S run after the fix: 222/222 from
# cache, 0 count queries (was 222), 0 failures, reference_df 4,061 seq / 1,104 spp,
# exactly the healthy baseline. The `total == 0L` early return is now guarded on
# `!any(is_cached)` -- without that, an all-cached call returned an EMPTY reference_df.
#
# P2 -- .fetch_fasta_cached(), one .rds per accession under <cache_dir>/fasta/. Warm
# Step 7a went from ~36 min to 3 s on all 221 genera, identical output.
#
# P5 -- EVICTION. Content-keying with no eviction does not REPLACE a cache when the key
# changes, it DOUBLES it: 1,584 of 3,517 meta files (45%) were orphaned by past key
# widenings. New .ref_cache_grammar() states, as a regex, every name .ref_cache_file()
# can produce; a file it does not match is unproducible for ANY arguments and so provably
# unreachable. KEEP THE TWO IN LOCKSTEP -- a widening mirrored in the grammar but not the
# key would delete LIVE files. test-fetch-cache-eviction.R generates names over a 288-cell
# argument grid and asserts every one matches; if it fails after a key change, fix the
# grammar in the SAME change, never relax the test. A differing key VALUE is never
# evictable (different length bounds, or the two rank_system sets, are different QUERIES
# and both are in live use). Automatic eviction is write-path only, scoped to the taxon
# just rewritten, capped at 5 MB/file; the whole-store sweep is
# taxalikely_evict_unreachable_cache(), dry_run = TRUE BY DEFAULT.
#
# THE fasta/ KEY WAS NOT VERSIONED, though its own docs said it was. `acc` comes from
# ESummary's `caption`, which carries no version (0 of 4,061 cached files had one). The
# surrounding machinery was correct and built FOR versioned input -- .parse_fasta_text()
# strips versions from headers, the write-back maps stripped ids to the requested string
# -- and both are silent no-ops on unversioned input, which is exactly why it stayed
# invisible. Fixed additively: .fetch_summaries_batched() gains acc_version (same batched
# call, no extra round trip), new .fasta_cache_keys() coalesces it over acc PER ROW, and
# the call site keys AND requests that string. `acc` deliberately stays UNVERSIONED --
# composite_id derives from it, .fetch_locations_batched() joins on GBSeq_primary-
# accession which has no version, and the dedupe collapses two revisions of one record.
# No meta key widened, nothing orphaned: a meta cached before acc_version existed cannot
# see a revision anyway, so falling back to the bare accession is correct, not a gap.
# devtools::test() 1283/0 (was 1213), check() 0/0/0. Live end-to-end run still OUTSTANDING
# -- NCBI began returning 500s then its throttling signature mid-session.
# Previous update: 2026-09-14, later (Opus 5): fetch_ncbi_reference_sequences() no longer
...
| `taxalikely_clear_cache()` | `R/taxalikely_clear_cache.R` | Written | **2026-09-04, new.** Reports/clears the `R_user_dir("TaxaLikely","cache")` directory `fetch_ncbi_reference_sequences()`/`audit_barcode_coverage()` write to (never expires automatically). `cache_dir`/`older_than_days`/`dry_run`, same interface as `TaxaFetch::taxafetch_clear_cache()` minus `orphans_only`. Built on the shared `TaxaTools::list_cache_files()`/`report_and_clear_cache()` engine. **CORRECTED 2026-09-14**: this row (and the file's own header) claimed "no orphan/duplicate mechanism exists here -- every cache key is deterministic 1:1 with its file." That is WRONG, and was being read as reassurance while 45% of the store was waste. Determinism is not the issue: the accumulation mechanism is KEY WIDENING -- each time a real staleness crash was fixed by adding a component to the key, the whole previous generation was orphaned rather than replaced (1,584 of 3,517 files). Use `taxalikely_evict_unreachable_cache()` for that, and this function only when you mean to clear everything. Also **2026-09-14**: now scans RECURSIVELY and recognises `_seq\\.rds$`, so it can finally reach the `fasta/` store P2 added -- 4,061 files that no clear function in the ecosystem could see. |
| `taxalikely_evict_unreachable_cache()` | `R/taxalikely_clear_cache.R` | Written | **2026-09-14, new (cache policy P5).** Removes ONLY the reference-cache files no call can ever hit again, leaving every reachable file in place -- as opposed to `taxalikely_clear_cache()`, the blunt instrument that targets everything. A file is targeted only if the current `.ref_cache_file()` construction cannot produce its name for ANY arguments (`.ref_cache_grammar()`), which is a proof rather than a heuristic: `rank_sfx` is appended unconditionally, so no argument combination yields a name lacking `_rk-`. Measured on the development machine: 1,584 of 3,517 (45%), 1.2 MB, of which 1,507 had a live counterpart for the same taxon+barcode. **`dry_run = TRUE` BY DEFAULT**, deliberately unlike every sibling `<pkg>_clear_cache()`: those are told what to delete, this one decides for itself, so it shows you first. `max_file_mb = 5` reports and spares anything large (a meta file is ~1 KB, so this is a structural guard, not an active filter). Two files differing only in a key VALUE -- `_l100_600` vs `_l100_5000`, or the two `rank_system` sets, both in live use -- are different QUERIES a caller may legitimately want both of, and are never targeted. |
