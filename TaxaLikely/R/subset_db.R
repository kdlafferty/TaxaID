...
#'     see \code{.pr2_hierarchy}'s roxygen for detail). Two real
...
subset_local_database <- function(fasta_path,
  ...
  if (!rank %in% rank_system) {
    stop(sprintf(
      "rank '%s' is not in rank_system (%s).",
      rank, paste(rank_system, collapse = ", ")
    ), call. = FALSE)
  }
  ...
  if (length(missing_cols) > 0L) {
    stop(sprintf(
      "taxonomy is missing rank column(s): %s.",
  ...
  message(sprintf(
    "Filtering taxonomy: %d unique taxon(s) at rank '%s'...",
  ...
  .save_record <- function() {
    if (!in_target_seq || is.null(current_id)) {
      return()
    }
    seq <- paste(current_seq, collapse = "")
    if (!is.null(max_n_bases) && nchar(seq) > max_n_bases) {
      return()
    }
