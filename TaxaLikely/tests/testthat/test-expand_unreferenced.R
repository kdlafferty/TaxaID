...
# Moved from TaxaAssign (Session 150) along with the function itself.
...
test_that("expand_unreferenced_hypotheses: score_likelihood_cov/evidence/h2_delta_source copied through to expanded H2/H3 rows", {
  ...
  h2 <- out[out$hypothesis_type == "unreferenced_species", ]
  expect_true(all(h2$score_likelihood_cov == 0.28))
  expect_true(all(h2$score_likelihood_evidence == 0.30))
  expect_true(all(h2$h2_delta_source == "genus_specific"))

  h3 <- out[out$hypothesis_type == "unreferenced_genus", ]
  expect_true(all(h3$score_likelihood_cov == 0.03))
  expect_true(all(h3$score_likelihood_evidence == 0.04))
  expect_true(all(h3$h2_delta_source == "global_fallback"))

  h1 <- out[out$hypothesis_type == "specific_candidate", ]
  expect_true(is.na(h1$h2_delta_source))
})

test_that("expand_unreferenced_hypotheses: constraint_applied (a genuinely row-specific extra column) stays NA on expanded rows", {
  lik <- .make_expand_lik()
...
# ---- observation_id-scoped unreferenced_df (Session 159 extension) ---------
# Lets restore_suppressed_candidates(check_regional_overlap = TRUE) inject a
# GLOBALLY-referenced species (would never appear via audit_barcode_coverage())
# that is nonetheless rejected for one specific query's region -- it should
# compete as a named H2 hypothesis only for that one observation_id.

test_that("expand_unreferenced_hypotheses: observation_id-scoped row expands only for that observation", {
  lik2 <- dplyr::bind_rows(
    .make_expand_lik(),
    dplyr::mutate(.make_expand_lik(), observation_id = "ESV_002")
  )
  ...
  out <- expand_unreferenced_hypotheses(lik2, unref_scoped)
  ...
  expect_true("Fundulus parvipinnis_REGIONAL" %in% h2_e1$taxon_name)
  expect_false("Fundulus parvipinnis_REGIONAL" %in% h2_e2$taxon_name)
  # Global rows (NA observation_id) still expand for both, as before.
  expect_true(all(c("Fundulus parvipinnis", "Fundulus zebrinus") %in% h2_e1$taxon_name))
  expect_true(all(c("Fundulus parvipinnis", "Fundulus zebrinus") %in% h2_e2$taxon_name))
})

test_that("expand_unreferenced_hypotheses: absent observation_id column behaves identically to a column of all NA", {
  out_absent <- expand_unreferenced_hypotheses(.make_expand_lik(), .make_unref_df())
  unref_na <- .make_unref_df()
  unref_na$observation_id <- NA_character_
...
test_that("expand_unreferenced_hypotheses: observation_id-scoped row for a species already H1 elsewhere does not leak into that other observation's suppression logic", {
  # ESV_001: F. lima is H1. ESV_002: F. lima is H1 too, but the regionally-
  ...
  out <- expand_unreferenced_hypotheses(lik_lima, unref_scoped)
  h2_e1 <- out[out$observation_id == "ESV_001" & out$hypothesis_type == "unreferenced_species", ]
  h2_e2 <- out[out$observation_id == "ESV_002" & out$hypothesis_type == "unreferenced_species", ]
