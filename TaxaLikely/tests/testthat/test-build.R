...
.make_four_genus_uniform_df <- function() {
  ...
  data.frame(
    ...
    species      = c("Aa bb", "Aa bb2", "Bb cc", "Bb cc2",
...
test_that("build_sequence_matrix: by_genus = TRUE non-logical/NA errors", {
  ...
  expect_error(build_sequence_matrix(df, c("genus", "species"), by_genus = "yes"),
               "by_genus")
  expect_error(build_sequence_matrix(df, c("genus", "species"), by_genus = NA),
               "by_genus")
})
...
test_that("build_sequence_matrix: by_genus = TRUE drops blank/NA-genus sequences with a message instead of erroring", {
  ...
  df2$genus[1] <- ""
  expect_message(
    out2 <- build_sequence_matrix(df2, c("genus", "species"), max_dist = 1.0,
                                  by_genus = TRUE, verbose = FALSE),
    "dropped 1 sequence\\(s\\) with blank/NA 'genus'"
  )
...
test_that("build_sequence_matrix: by_genus = TRUE errors when dropping blank-genus rows leaves fewer than 2 sequences", {
  ...
  expect_error(
    build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                          by_genus = TRUE, verbose = FALSE),
...
test_that("build_sequence_matrix: by_genus = TRUE representative draw is reproducible via set.seed()", {
  ...
  set.seed(7L)
  out1 <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                by_genus = TRUE, verbose = FALSE)
  set.seed(7L)
  out2 <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                by_genus = TRUE, verbose = FALSE)
...
test_that("build_sequence_matrix: by_genus = TRUE and FALSE give the same within-genus pairs", {
  ...
  out_whole <- build_sequence_matrix(df_aa, c("genus", "species"), max_dist = 1.0,
                                     verbose = FALSE)
  set.seed(3L)
  out_by_genus <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                        by_genus = TRUE, verbose = FALSE)
  ...
  expect_equal(sort(paste(within_genus$id_x, within_genus$id_y)),
...
test_that("build_sequence_matrix: max_foreign_reps_per_genus non-numeric/negative/NA errors", {
  df <- .make_three_genus_df()
  expect_error(build_sequence_matrix(df, c("genus", "species"), by_genus = TRUE,
...
test_that("build_sequence_matrix: max_foreign_reps_per_genus caps cost between the 0 and uncapped extremes", {
  ...
  set.seed(1L)
  out_uncapped <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                        by_genus = TRUE,
                                        max_foreign_reps_per_genus = NULL,
                                        verbose = FALSE)
  set.seed(1L)
  out_cap1 <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                    by_genus = TRUE, max_foreign_reps_per_genus = 1L,
                                    verbose = FALSE)
  set.seed(1L)
  out_cap0 <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                    by_genus = TRUE, max_foreign_reps_per_genus = 0L,
                                    verbose = FALSE)
...
test_that("build_sequence_matrix: max_foreign_reps_per_genus default (20L) behaves as uncapped when it exceeds genus count", {
  ...
  df <- .make_three_genus_df()
  set.seed(3L)
  out_default <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                       by_genus = TRUE, verbose = FALSE)
  set.seed(3L)
  out_uncapped <- build_sequence_matrix(df, c("genus", "species"), max_dist = 1.0,
                                        by_genus = TRUE,
...
test_that("check_cross_genus_sampling_noise: returns the documented list(replicates=, summary=) shape", {
  ...
  df <- .make_two_genus_df()
  set.seed(11L)
  result <- check_cross_genus_sampling_noise(df, rank_system = c("genus", "species"),
  ...
  expect_true(all(c("replicate", "n_cross_genus_pairs", "mean_p_match",
                    "median_p_match", "sd_p_match") %in% names(result$replicates)))
...
test_that("check_cross_genus_sampling_noise: errors clearly when rank_system lacks genus", {
  ...
  df <- .make_two_genus_df()
  expect_error(
    check_cross_genus_sampling_noise(df, rank_system = "species", max_dist = 1.0,
                                     n_replicates = 2L),
    "genus"
  )
})
...
test_that("check_cross_genus_sampling_noise: n_replicates < 2 errors", {
  df <- .make_two_genus_df()
  expect_error(
    check_cross_genus_sampling_noise(df, rank_system = c("genus", "species"),
                                     n_replicates = 1L),
    "n_replicates"
  )
})
