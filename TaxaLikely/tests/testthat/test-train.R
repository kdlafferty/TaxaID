...
test_that("flag_reference_errors: singleton_match_threshold is a real, working parameter", {
  ...
  out_lower <- flag_reference_errors(.make_raw_df(),
    return_all = TRUE,
...
test_that(".prep_training_data: returns data frame with expected columns", {
  ...
  expect_true(all(c(
    "id_x", "score_logit", "gap_logit", "rank_category", "N_Obs",
    "rank_code_a"
  ) %in% names(out)))
})
...
test_that("train_likelihood_model: H1_Lookup$sigma_score is a true variance, not its square root (Session 158)", {
  ...
  expect_false(isTRUE(all.equal(
    out$H1_Lookup$sigma_score[aa_idx],
...
.make_genus_raw_df <- function() {
  ...
  grid$p_match <- mapply(
    function(x, y, sx, sy, gx, gy) {
      if (x == y) {
        return(1.00)
      } # self-match
      if (sx == sy) {
        return(0.97)
      } # within-species, different sequence
      if (gx == gy && gx == "Fundulus") {
        return(0.90)
      } # tight true congener
      if (gx == gy && gx == "Loose") {
        return(0.75)
      } # looser true congener
...
test_that("train_likelihood_model: verified_clean is forwarded to the internal flag_reference_errors() call", {
  ...
  model_default <- train_likelihood_model(df,
  ...
  expect_equal(model_default$reference_errors$error_type[
    model_default$reference_errors$id_x == "A1"
  ], "likely_mislabeled")

  model_verified <- train_likelihood_model(df,
    rank_system = c("genus", "species"),
    use_hierarchy = FALSE, anchor_perfect = FALSE,
    verified_clean = "A1"
  )
...
test_that("train_likelihood_model: mislabel_behavior default ('flag') keeps a flagged accession's pairs in training; 'remove' still drops them", {
  ...
  out_flag <- train_likelihood_model(df,
    rank_system = c("genus", "species"),
    use_hierarchy = FALSE, anchor_perfect = FALSE
  )
  out_remove <- train_likelihood_model(df,
    rank_system = c("genus", "species"),
    use_hierarchy = FALSE, anchor_perfect = FALSE,
