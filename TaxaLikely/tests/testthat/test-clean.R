...
test_that("remove_flagged_references removes singletons when requested", {
  ...
  out <- suppressMessages(remove_flagged_references(
    match_df, errors,
    remove_unverified_singletons = TRUE
  ))
...
test_that("remove_flagged_references validates inputs", {
  expect_error(
    remove_flagged_references("not_a_df", data.frame(id_x = "a", error_type = "b")),
    "match_df must be a data frame"
  )
  expect_error(
    remove_flagged_references(data.frame(x = 1), "not_a_df"),
    "reference_errors must be a data frame"
  )
  expect_error(
    remove_flagged_references(data.frame(x = 1), data.frame(x = 1)),
