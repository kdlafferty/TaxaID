...
| 2026-05-23 | *(Session 85 — new)* | `call_api()` | TaxaTools | function | Generic LLM dispatcher in `R/call_api.R`. Single exported function handles all providers via data-driven dispatch through `inst/model_tiers.json`. Three handler families: `anthropic`, `gemini`, `openai_compat`. Params: `provider`, `tier`, `model`, `max_tokens`, `api_key`, `base_url`. Attaches `model` + `provider` attributes. |
| 2026-05-23 | *(Session 85 — refactor)* | `call_anthropic_api()`, `call_gemini_api()`, `call_openai_api()`, `call_azure_api()`, `call_ollama_api()` | TaxaTools | thin wrappers | All five provider functions converted to thin wrappers around `call_api()`. Same signatures, same behavior; HTTP logic now lives in `call_api.R`. Kept for backward compatibility and `llm_fn` use. |
| 2026-05-23 | *(Session 85 — new)* | `inst/model_tiers.json` | TaxaTools | data file | New fields: `handler_family` (drives dispatch), `chat_endpoint`/`chat_endpoint_template` (URL resolution), `api_version_header` (Anthropic), `body_max_tokens_key` (Azure o-series uses `max_completion_tokens`). New `ollama` provider entry. |
| 2026-05-23 | *(Session 85 — new)* | `options(TaxaID.provider)` | Ecosystem | global option | New R option storing the active provider name string (e.g. `"anthropic"`). Set by `.onAttach()` alongside `TaxaID.llm_fn = call_api`. `call_api()` reads this when no explicit `provider` arg. Change with: `options(TaxaID.provider = "gemini")`. |
...
| 2026-05-26 | *(Session 87 — new)* | `read_birdnet_output()` | TaxaMatch | function | Ingest BirdNET-Analyzer CSV results into match object format. Accepts file vector or directory. `observation_id = "{file_stem}_{start_s}-{end_s}"`. `min_confidence` + `top_n` filters. 9 offline tests using synthetic BirdNET data. |
| 2026-05-26 | *(Session 87 — new)* | `fetch_reference_recordings()` | TaxaLikely | function | Fetch bird sound recordings from Xeno-canto for acoustic reference training. `api_key` param (reads `XC_API_KEY` env var). Returns metadata + optional audio download. Acoustic analog to `fetch_reference_sequences()`. |
| 2026-05-26 | *(Session 87 — breaking)* | Xeno-canto API v2 → v3 | TaxaLikely | `fetch_reference_recordings()` | v2 endpoint retired (404). v3 endpoint (`/api/3/recordings`) requires API key. `api_key` param added; `per_page = 500L`; 401 handled with clear registration message. Online tests guarded by `skip_if(nchar(Sys.getenv("XC_API_KEY")) == 0L)`. |
| 2026-05-26 | *(Session 87 — new)* | `call_api_pdf()` (formerly `call_anthropic_api_pdf`) | TaxaFetch | function | Renamed and generalized to support all vision-capable providers via `call_api(images=)`. `provider`, `tier`, `base_url` params added. |
...
| 2026-05-27 | *(Session 93 — new)* | `build_image_reference()` | TaxaLikely | function | Image analog of `build_acoustic_reference()`. Joins image classifier detections (`image_df`) to user-supplied ground-truth `images_meta` (requires `image_path` + rank cols). Join key: `observation_id` (file stem) matches `file_path_sans_ext(basename(image_path))`. `coverage` from `image_df$coverage` (bbox area) or `images_meta$quality`. `testid` from `images_meta$testid`. 27 tests. |
| 2026-05-27 | *(Session 93 — new)* | `read_inaturalist_cv_output()` | TaxaMatch | function | Read per-image JSON files saved from the iNaturalist CV API into match object format. `score_type = "combined_score"` / `"score"`. Returns `observation_id`, `score`, `species`, `genus`, `common_name`, `taxon_rank`, `source_file`. `min_confidence`, `top_n`. Requires `jsonlite`. |
| 2026-05-27 | *(Session 93 — new)* | `read_wildlife_insights_output()` | TaxaMatch | function | Read SpeciesNet / Wildlife Insights batch predictions JSON (top-level `"predictions"` dict keyed by image filename). `label_col = "label"`, `score_col = "score"` configurable. Returns `observation_id`, `score`, `species`, `genus`, `category`, `source_file`. Requires `jsonlite`. |
| 2026-05-27 | *(Session 93 — new)* | `inst/workflows/3c_image_reference_workflow.R` | TaxaLikely | workflow | Image reference training workflow: reference image acquisition → classifier → `read_animl_output()` → `build_image_reference()` → `train_likelihood_model()` per testid. 9 steps mirroring Workflow 3b (acoustic). |
| 2026-05-27 | *(Session 93 — dependency)* | `jsonlite` added to Imports | TaxaMatch | dependency | Required by `read_inaturalist_cv_output()` and `read_wildlife_insights_output()` for JSON parsing. |
| 2026-05-28 | *(Session 94 — new)* | `write_reference_fasta()` | TaxaLikely | function | Export any `reference_df` to FASTA + optional companion taxonomy TSV (positional format; round-trippable via `read_reference_fasta(taxonomy_file=)`). FASTA header: `>{composite_id} {rank1_val} ... {rankN_val}` (NA ranks omitted). `rank_system` auto-detected from non-id columns when NULL. |
| 2026-05-28 | *(Session 94 — new)* | `build_site_reference()` | TaxaLikely | function | High-level site-specific DNA reference builder: taxa list (from TaxaExpect or user) → `fetch_reference_sequences()` → optional `flag_reference_errors()` → `audit_barcode_coverage()` → `write_reference_fasta()`. Returns `$reference_df`, `$errors`, `$census`, `$unreferenced`. `output_dir` writes `reference.fasta` + `reference_taxonomy.tsv`. DNA-only (eDNA/barcode). |
| 2026-05-28 | *(TODO — next session)* | Image processing end-to-end workflow | TaxaMatch + TaxaLikely + TaxaAssign | workflow | **TODO: Create and test a complete image classification workflow.** Covers: obtain labeled reference images → run Animl/SpeciesNet → `build_image_reference()` → `train_likelihood_model()` → evaluate field images → `evaluate_likelihoods()` → `TaxaAssign`. Needs real or synthetic test data to validate the full pipeline end-to-end. See `inst/workflows/3c_image_reference_workflow.R` (training) and Workflow 4 (field evaluation). |
| 2026-05-28 | *(TODO — future)* | Camera trap image reference library | TaxaLikely | function | **TODO: Investigate `build_camera_trap_reference()`.** Users' own camera trap images with verified species IDs are a primary image reference — different from downloading iNaturalist images. This would enable site-calibrated image reference sets analogous to `build_site_reference()` for DNA. Design decision needed: how to manage a local image library metadata index (CSV/RDS pointer file vs actual image copies). |
| 2026-05-28 | *(Session 95 — expansion)* | TaxaWizard acoustic/image/local-reference | TaxaWizard | graph + metadata | `workflow_graph.json`: 4 new input nodes (`birdnet_detections`, `image_classifier_output`, `local_fasta`, `images_meta`), 2 new intermediate nodes (`acoustic_matrix`, `image_matrix`), 8 new edges. 6 new code snippets. TaxaMatch.json: +4 functions (`read_birdnet_output`, `read_animl_output`, `read_inaturalist_cv_output`, `read_wildlife_insights_output`). TaxaLikely.json: +9 functions + updated `read_reference_fasta` (taxonomy_file param). `phase_classify.md`: new CRITICAL DISAMBIGUATION rules for all new input types. |
| 2026-05-28 | *(Session 95 — cleanup)* | Monorepo file cleanup | Ecosystem | infrastructure | Removed 7 `.Rcheck/` dirs, 3 build tarballs, duplicate `TaxaLikely/TaxaLikely/` dir (140+ MP3s), 30+ root-level dev cache files, `openalex_cache/`, dated workflow files. Moved `score_accuracy_demo.R` to `TaxaLikely/inst/`. Fixed spelling: `workflow_accoustics.R` → `workflow_acoustics.R`. Consolidated `run_birdnet.py` to `TaxaMatch/inst/`. `.gitignore` updated with `reference_audio/`, `birdnet_results/`, `.Rapp.history`, `Rplots.pdf`, `taxaid_methods_*.md`. |
| 2026-05-28 | *(Session 95 — docs)* | README Pipeline Overview DAG | TaxaID | README.md | Added Mermaid flowchart showing all supported workflow paths (eDNA, acoustic, image, reference building, priors, assignment). Fixed stale "next planned" text for `build_image_reference()`. Updated Software Inventory counts (all 9 packages). Updated TaxaWizard README with full workflow path list. |
