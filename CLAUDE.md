...
# workflow_structure_outlines_2026_09_15.txt (per-section banner, step message, resolved
# pkg::function set and .save() checkpoints for all 8 files). Remaining items:
# ecosystem_docs/REENTRY_PROMPT_workflow_audit_followups.md.
#
# THE AUDIT DOC'S OWN FILE INVENTORY WAS STALE IN FOUR WAYS, which is the reason to run
# this off `git log -p` rather than a session note: PtConceptionWorkflow_12S_multi_site.R
# (retired 1bba8c5) and MuguWilderFishWorkflow.R (archived, and .gitignore's _archive*/
# means an archived script silently LEAVES version control) were both still listed as
# live; _12S_multi_site_FAST.R and CaliforniaIntertidalWorkflow_multi_marker.R were not
# listed at all. Every surviving file had grown 15-30%. Its Safety-notes bullet also still
# said "None of these 9 files are under git" -- false in THREE repos now, since the TaxaID
# monorepo itself holds the generic template. Corrected in place.
#
# BOTH OF ITS "TWO REAL FINDINGS" CLOSED, NEITHER BY FIXING THEM. The lab_contaminant_risk
# latent break needed no action (one file retired, the other already corrected; a sweep of
# all 8 live files found ZERO surviving filters -- the remaining hits are comments plus a
# deliberate back-compat shim at 18S 2037-2050). The 18S "missing Section 6" was never real:
# it is at line 1449, labelled 2026-09-07, with an inline comment explaining that
# standardize_match_data() legitimately moved to Step 1 because that marker's GBIF fetch
# needs backbone-resolved taxonomy. The original survey's regex missed it -- and the first
# extractor written this session made the mirror-image error, reading two `# 18S ...` comment
# lines as section banners, which is why the banner regex now requires the rule/title/rule
# triple. Check a structural regex against a file you have read before trusting its output.
#
# ONE LATENT BREAK FOUND AND FIXED IN THE CANONICAL TEMPLATE: eDNA/PtConception/
# TaxaID_eDNA_Workflow_Template.R:280 called TaxaFetch::define_search_polygon(); that
# function is exported by TaxaTools (TaxaFetch's own Rd cross-references say so), so the
# template threw at its first GBIF step. All three production scripts already had it right
# -- the same shape as the lab_contaminant_risk finding, a rename that reached the scripts
# and not their template. A namespace sweep over all 8 files found no other invalid
# pkg::fun call except the retired template's and Mugu's documented dead else-branch.
#
# TaxaID_Workflow_Template_TEST.R RETIRED (user decision) to
# archive_retired_workflow_template_2026_09_15/ with a README; git rm'd from inst/,
# .Rbuildignore'd (verified: R CMD build tarball contains 0 references), archive untracked
# per .gitignore, matching TaxaExpect/archive_glmm_prior_pipeline and the 1bba8c5 precedent.
# WHY: its Section 5 called seven functions -- optimize_grid_size, create_sites_from_grid,
# prepare_model_dataframe, compute_moran_basis, screen_spatial_formula, generate_full_priors,
# plot_theta_map_interactive -- that moved to TaxaExpect/archive_glmm_prior_pipeline/R/ on
# 2026-09-09 with the GLMM chain. They are ARCHIVED, NOT LOST, and this file was that
# chain's LAST REMAINING CALLER. The calls are unguarded, so it stopped at L939; contrast
# MuguFishWorkflow.R 890-901, which calls three of the same functions inside a hardcoded
# USE_KERNEL_PRIORS <- TRUE else-branch annotated "no longer functional" -- documented dead
# code, left alone. It was last committed 2026-09-14, the same day as every running
# workflow, because broadcast patches kept touching it: the exact state 12S_multi_site.R was
# in ("kept receiving ecosystem-wide patches, so it looked alive"). A patch to Sections 7-8
# says nothing about whether Section 5 runs. It also had no Step 1, no match-standardisation
# step, no filter/output step, and numbering off by one against every production script
# (its Step 6 = TaxaLikely; production Step 7 = TaxaLikely).
# The root README.md had been advertising it as "a genuinely runnable, self-contained worked
# example that exercises the full pipeline end to end" and the way to "confirm your
# installation and API keys work" -- FALSE since 2026-09-09, so a new user following it would
# have hit a namespace error and blamed their own install. Corrected, along with
# TaxaMatch/inst/workflows/score_acoustic_workflow.R's cross-reference and a .gitignore
# comment. NAME_CHANGE_HISTORY.md entry added. OPEN GAP, recorded not papered over: the
# TaxaID package now bundles NO end-to-end runnable example; the canonical template lives in
# the separate eDNA repo and is itself behind on four subsystems (below).
#
# THE CANONICAL TEMPLATE IS BEHIND ON FOUR SUBSYSTEMS that all three single-marker production
# scripts have and it has none of: kernel priors (calibrate_kernel_bandwidth/
# estimate_kernel_priors/plot_theta_surface), the whole reference screen
# (fetch_ncbi_reference_sequences + corroborate_references_locally ... remove_incongruent_
# references), calibrate_query_noise, and ALL of report assembly (report_fetch/habitat/priors/
# likelihood/assign/flags + assemble_report + generate_report). Also missing
# compute_group_priors and flag_watch_candidates from its Section 8. It is not abandoned --
# last touched 2026-09-14 -- it just never received those migrations.
#
# TWO GAPS IN GREATLAKES, both unexplained rather than documented: it makes 12 LLM calls and
# calls token_usage()/reset_token_usage() ZERO times, so its runs carry no cost record while
...
# collapses Mugu's Round-1/cross-marker/Round-2 trio into one Section 8), so "Step 9" means
# TaxaFlag review there and in the single-marker family, and Round-2 Bayes in Mugu. That is
# arguably the better convention and nothing records which is intended. CalIntertidal is a
# SCAFFOLD (31 explicit `## STUB` markers, no data yet), so its gaps are not drift.
#
# TAXAWIZARD GRAPH vs GROUND TRUTH, measured but deliberately NOT wired (the audit doc warns
# that wiring an unverified outline into a hand-audited graph risks the opposite of the goal):
# the graph is internally SOUND -- all 65 functions named across its 32 edges are genuinely
# exported, 0 invalid references, so the NAMESPACE hand-sync works. But the five running
# workflows call 89 ecosystem functions and 41 appear in NO edge; EIGHT of those are called by
# all five, so they are unambiguous gaps rather than site-specific extras:
# add_posthoc_assessment, adjust_inat_range_priors, apply_coverage_constraints,
# call_anthropic_api, expand_unreferenced_hypotheses, fill_higher_ranks, flag_watch_candidates,
# scientific_to_common. The 17 graph functions no workflow uses are NOT errors -- they are
# TaxaWizard's alternate entry points (BirdNET, image classifiers, CRABS/local FASTA, the
# run_*_pipeline wrappers) that no eDNA site exercises.
#
# ALSO CAPTURED THIS SESSION, from a parallel session that had not reached any CLAUDE.md:
# eDNA/PtConception/Multi_site_multi_marker/ (~20 scripts) rebuilds the Pt Conception sample
# metadata and is DATA-AFFECTING, not just code. Read its own
# CHANGES_2026-09-16.md before trusting any derived table built before 2026-09-16 -- six
# numbered changes, of which these change VALUES: a trailing-space crosswalk key made Event 5
# read 32 samples as 52 and Event 8 read 30 as 50; one build ingested its own output as 738
# phantom samples (a changelog reading "13 events, 1516 samples" is that build -- discard it);
# Coal Oil Point barcodes IE7HZVSJ/J9K1AONX carried a waypoint 500 km north (Duxbury Reef),
# now 34.40657/-119.87790; two Event 5 barcodes moved sub-site on Jessie Altstatt's
# confirmation; gp_area was silently dropped by a select(); the `corrected` column was
# garbage. PtConception/ptcon_ingest.R moved into that folder.
#
# Previous update, 2026-09-15 (Opus 5): RANK-AGREEMENT GATE added to both backbone
# harmonisers -- TaxaTools::assign_sampling_group(harmonise = TRUE) and
# CaliforniaIntertidal/scope_classifier.R's harmonize_ranks_to_gbif(). Closes
# ecosystem_docs/REENTRY_PROMPT_harmonise_rank_agreement.md.
#
# THE BUG: both functions resolve a row's finest RANK NAME against GBIF and overwrite the
# whole lineage with whatever comes back, never checking verify_taxon_names()'s matched_rank.
# GBIF's backbone holds a tachinid FLY GENUS named "Polychaeta"
# (Animalia|Arthropoda|Insecta|Diptera|Tachinidae|Polychaeta, verified live), so a class
# column reading "Polychaeta" became Arthropoda/Insecta + a fabricated order Diptera.
# On the 2026-09-15 PtCon 18S run: 7 marine polychaete taxa, 17 rows, all grouped
# terrestrial_arthropods and then SILENTLY DROPPED by classify_18S_functional()'s marine
# filter. Confirmed against the shipped export -- 0 Insecta rows, the 7 taxa simply absent.
#
# THE GATE (two rules, both exact): accept a resolution when (1) matched_rank equals the
# probe COLUMN's rank -- the ordinary case, and the only thing that can vouch for a SYNONYM
# resolution, where the returned name is deliberately not the name asked about; or (2) the
# rank differs but the returned lineage carries the probe name ITSELF at the probe's own
# rank -- a same-clade duplication (GBIF has a genus Spionidae inside family Spionidae, a
# genus Arthropoda inside phylum Arthropoda), not a homonym. Anything else keeps its
# ORIGINAL taxonomy, the same fallback an unresolved name already got, and warns by name.
#
# POLYCHAETA WAS NOT ALONE. Scanning all 288 (name, rank) pairs in that run's NCBI-side
# taxonomy found 15 rank disagreements. Rule 2 rescues 8 (Arthropoda + 7 polychaete family
# names). Of the 7 rejected, FIVE are genuine cross-lineage homonyms that would each have
# rewritten a whole lineage: Polychaeta (class -> tachinid fly genus), Ctenophora (phylum ->
# crane-fly genus), Ciliophora (phylum -> a FUNGUS genus, Fungi|Ascomycota), Appendicularia
# (class -> a flowering-plant genus), Pilidiophora (class -> a gregarine genus). Only
# Polychaeta was ever noticed, because its taxa were dropped; a corrupted lineage that stays
# marine is silent. The other two rejections, Bacillariophyta (NCBI phylum / GBIF class) and
# Bigyra (NCBI class / GBIF phylum), are real NCBI-vs-GBIF rank-assignment differences for
# the same clade -- rejecting them is harmless (the diatom rule is class-level by design and
# classify_18S_functional() keys diatoms on Bacillariophyceae, never on that phylum value).
#
# CACHES WRITTEN BEFORE THE GATE CARRY NO matched_rank and so cannot be checked -- both
# functions DISCARD such a cache and re-resolve rather than trust it, since re-running with
# an old cache_dir is exactly when a silent re-admission would go unnoticed. This already
# applies to PtCon18SSchulte_sampling_group_harmonise_cache/ and CaliforniaIntertidal/
# gbif_rank_lookup.rds, both of which hold the corrupted Polychaeta lineage today.
#
# VERIFIED by replaying the real 18S Step 10 from PtCon18SSchulte_reviewed.rds: all 7 taxa
# and 17 rows now group macroinvertebrates AND pass the marine filter; macroinvertebrates
# 491 -> 508, phytoplankton 955, macroalgae 190, zooplankton 312, parasites 1 -- unchanged.
# THE 18S EXPORT ON DISK IS STILL THE OLD ONE; Step 10 has not been re-run for real.
# 6 new offline tests in TaxaTools/tests/testthat/test-sampling_group.R (63 pass, 0 fail);
# R CMD check TaxaTools = 1 error, PRE-EXISTING and unrelated (vignettes/name-cleaning.Rmd
# 'match_df' not found), 0 warnings, 0 notes.
#
# Previously: 2026-09-15 (Opus 5): prior_branch RENAMED "resident_observed" ->
# "kernel_estimated", and compute_group_priors()'s rank_cols widened to reach order/class.
# Item 2 of the user's running order (REENTRY_PROMPT_resident_observed_evidence_gate.md).
#
# THE RENAME: TaxaExpect::estimate_kernel_priors() writes prior_branch as a CONSTANT on
# every row it emits, so the label records which GENERATOR made the row, never how much
# evidence stands behind it -- yet two consumers read it as a real evidence claim.
# Measured: within that one branch effective_records spans ~10 orders of magnitude, and
# 215 of 479 real PtCon 12S rows (44.9%) sit under ONE Kish effective record, minimum
# 0.0000; PtCon 18S 987/1521 (64.9%); GreatLakes only 17/90 (18.9%). The distribution is
# CONTINUOUS with no valley, so no threshold can be read off the data -- which is why the
# user chose rename + an explicit evidence knob over a gate. posterior_consensus() gains
# min_effective_records, DEFAULT 0 = branch-only = byte-identical to before; raising it
# moves plausibility categories and needs the Lamar re-check first (benchmark 0.872).
#
# BOTH STRINGS ARE ACCEPTED EVERYWHERE, permanently, not transitionally: every prior table
# checkpointed before today carries the old name across four sites, and a reader that knew
# only the new one would silently reclassify all of them -- the exact failure the rename
# exists to stop. New .KERNEL_BRANCH/.is_kernel_branch() in TaxaAssign/R/kernel_branch.R is
# the single place that knows. Verified on the real PtCon table: 497 of 792 rows kept,
# identical to the pre-rename filter. SEVENTEEN hardcoded `== "resident_observed"` filters
# across 6 workflow files were patched too -- several load-bearing (.not_clamp drives the
# species_reference downranking filter), and they would have matched ZERO rows on the next
# fresh priors run while looking fine on every cached one.
#
# THE RANK GAP (folded in by user decision, same defect in the same signal): rank_cols
# stopped at "family", so a consensus resolving at ORDER rank found no group row, read
# consensus_has_occurrence_record = FALSE and was auto-reported "unprecedented" -- the same
# bug as the documented Mugu 504/616 species-rank case, one rank coarser. Real cost, found
# while closing item 1: the Perciformes unit (Scorpaenichthys marmoratus + Hexagrammos,
# 20 observations) was flagged unprecedented, TaxaFlag's skepticism gate then required the
# reviewer to rate it geographically "unlikely" -- which it did while its OWN review_comment
# called the candidates "very common at Pt. Conception" -- and the export filters dropped
# all 20. Cabezon and kelp greenling are Point Conception natives. Default now reaches
# order/class; a DEFAULT rank absent from taxonomy_map is skipped with a message, a rank
...
#
# THE RULE WORTH CARRYING: an uncached, network-dependent query must never gate a cached
# payload. The 2026-09-14 incident was cache-BYPASS ORDERING, not missing caching -- all 7
# genera whose NCBI count query failed had valid cached metadata on disk; the count loop ran
# unconditionally BEFORE the cache check, an NA count zeroed retmax_cap, and the taxon was
# dropped before its cache file was ever consulted. Cost: 78 sequences, 17 species. P1
# reorders it; the fixed run reproduces the healthy 4,061 seq / 1,104 spp exactly with 0
# count queries. Warm Step 7a: ~36 min -> 3 s.
#
# SECOND RULE: content-keying WITHOUT eviction does not replace a cache when the key
# changes, it DOUBLES it. 45% of TaxaLikely's meta store (1,584 of 3,517) was orphaned by
# past key widenings, each of which was itself a correct fix for a real staleness crash. P5
# now evicts, on a PROOF rather than a heuristic -- .ref_cache_grammar() states every name
# the key can produce, and a file it cannot produce is unreachable for any arguments. Keep
# grammar and key in lockstep; a grammar widened without the key deletes LIVE files, and a
# 288-cell generated-argument test pins them together.
#
# THREE COMMENTS ASSERTED A STALENESS AXIS WAS CLOSED WHEN IT WAS OPEN, and all three were
# corrected from data rather than from the docstring: the 12S workflow's "can never again
# silently serve stale reference data"; taxalikely_clear_cache.R's "no known duplicate/
# orphan-accumulation mechanism here"; and .fetch_fasta_cached()'s "keyed on the FULL
# accession including its version suffix" (it was keyed on ESummary's `caption`, which
# carries no version -- 0 of 4,061 cached files had one). The surrounding fasta machinery
# was correct and written FOR versioned input, which is precisely why the gap stayed
# invisible: every version-handling step is a silent no-op on unversioned input.
#
# ALSO FOUND: a GBIF geometry key hashed as nchar(geometry), so editing a bbox coordinate
# without changing its length served the WRONG REGION; P2's own fasta/ store (4,061 files)
# and a 122 MB hand-quarantined zip were both unreachable by every clear function in the
# ecosystem. All fixed. THREE OF THE REVIEW'S OWN FINDINGS WERE WRONG and were withdrawn
# after checking what each thing is FOR (TaxaMatch needs no clear function -- row-level and
# TTL'd; TaxaExpect's cross-package cache_dir default is correct; taxatools_clear_cache()
# having no default is correct). CLOSED, do not reopen: the single-site reference_df read
# stays unbuilt -- the entire available win is under 3 seconds against a second staleness
# surface. Tests TaxaTools 992/0, TaxaLikely 1283/0, TaxaFetch 810/2 (both pre-existing
# CoordinateCleaner). check() 0/0/0 on all three. NOTHING DELETED YET: the sweep defaults to
# a dry run and the zips need an explicit call. OUTSTANDING: a live end-to-end
# fetch_ncbi_reference_sequences() run confirming versioned filenames land in fasta/ --
# attempted 3x, abandoned when NCBI returned 500s then its throttling signature.
#
# Previous update: 2026-09-14 (Opus 5 -- OVERNIGHT PtConception 12S PRODUCTION RUN, then one
...
# ESV's top score, a taxon reported at a rank only when it holds >= 90% of the HITS in that
# bracket, widen to 2% when a 97%-or-better match still returns no family. THE OLD DOCS WERE
# WRONG: score_consensus()'s roxygen claimed the gap-style algorithm reproduced "the GITA /
# Jonah Ventures convention"; it reproduces the generic fixed-threshold/LCA convention, and
# the false attribution is removed. This matters for the benchmark paper -- strict unanimity
# upranks MORE readily than a 90% rule, so a TaxaID-vs-score_consensus() comparison run in
# gap mode is biased toward TaxaID on resolution (same-name rate vs JV's delivered taxonomy:
# 0.9999 at agreement_fraction 0.9 vs 0.9957/0.9918 at 1.0 -- unanimity disagrees ~60x more
# often). ecosystem_docs/AQUARIUM_BENCHMARK_DESIGN.md and TODO_validation_benchmark.md both
# proposed gap mode AS the JV benchmark and were corrected. Validated at 14,718/14,719 ESVs
# (0.9999) against JV's OWN delivered Pt Conception MiFish taxonomy via
# diagnostics/jv_bracket_consensus_validation.R, run on JV's own detailed-hit tables so the
# reference library is held constant and any disagreement is the RULE, not the database.
# Standing lesson recorded in the roxygen: when a provider ships its own consensus taxonomy,
# benchmark against THAT, not against a reconstruction of their rule. Three JV ambiguities
# decided and flagged for the paper -- see TaxaAssign/CLAUDE.md's top session note.
# Previous update, 2026-09-10, later (Fable 5.1 -- the H1 coverage-floor decision from the
# entry directly below is BUILT, VALIDATED and DEFAULT-ON: TaxaLikely::train_likelihood_
# model(min_pair_coverage = 0.8, shrinkage = "empirical_bayes"); TaxaMatch::blast_
# sequences() records min_query_coverage in report_params; evaluate_likelihoods() checks
# the two. Validated on the REAL GreatLakes workflow's own code path (7b.5 -> 8h re-executed
# per arm against today's checkpoints, control reproduces production 884/885): Lamar
# co-detections 593 -> 798, precision 0.805 -> 0.818, 29 -> 41 of 61 species, 0 lost, grass
# carp back. See TaxaLikely/CLAUDE.md's top note and the Recent Breaking Changes row.
# Previous update, 2026-09-10 (Fable 5.1 -- FIRST FULL GREATLAKES RUN on the rewired reference
...
| 2026-09-10 (Fable 5.1) | `train_likelihood_model(min_pair_coverage = 0.8, shrinkage = "empirical_bayes")` added, BOTH default-on | TaxaLikely | **Behavioral default change for every caller whose `raw_df` carries a `coverage` column (every `build_sequence_matrix()` output).** `min_pair_coverage`: a pair must have coverage >= 0.8 before it may DEFINE a reference's best foreign/congener/conspecific match -- no pair removed, no species dropped (self-side fallback). Reason: 86% of real GreatLakes 12S references had a 100%-identity, 4.6%-coverage "best foreign match", so the trained gap was negative for 89% of references and the H1 likelihood was a near-tie on 82% of ASVs. Must equal `blast_sequences(min_query_coverage)/100` (now recorded in the match object; `evaluate_likelihoods()` warns on mismatch). `shrinkage = "empirical_bayes"`: per-species H1 means shrunk with an estimated between-species variance instead of the fixed `N/(N+prior_weight)` (`"fixed"` restores it exactly). Validated on the real GreatLakes workflow code path: Lamar co-detections 593 -> 798, precision 0.805 -> 0.818, species 29 -> 41 of 61, none lost. Any `lik_model.rds` trained before this is stale and should be retrained. `H1_Lookup` gains `shrink_w_score`/`shrink_w_gap`; `Stats` gains `shrinkage`/`tau2_score`/`tau2_gap`/`min_pair_coverage`/`n_self_fallback`/`n_foreign_unqualified`. |
| 2026-09-10 (Fable 5.1) | `blast_sequences()` `report_params` gains `min_query_coverage` | TaxaMatch | Additive. Read by `TaxaLikely::evaluate_likelihoods()` for the train/inference coverage check. |
| 2026-09-10 (Fable 5.1) | `download_gbif_occurrences()` gains `submit_attempts`/`submit_wait`/`on_submit_failure` | TaxaFetch | Additive; behavioral only when GBIF refuses the download request (a real "HTTP 503 Backend fetch failed" killed a GreatLakes run at Step 3 with a verified identical zip on disk). Transient 5xx retried with backoff; with `overwrite = TRUE` and a verified cache, the cache is imported with a loud warning and `attr(out, "served_from_cache_after_failure") = TRUE` rather than the run dying; `"error"` restores the old behaviour. Non-transient errors (401) are never retried or masked. |
| 2026-09-12 (Opus 5) | `score_consensus()` gains `consensus_mode` (`"gap"`/`"bracket"`), `agreement_fraction`, `bracket_width`, `bracket_fallback`; new `bracket_width_used`/`agreement_achieved` columns | TaxaAssign | **Additive, fully backward compatible -- and a DOCUMENTATION CORRECTION that matters for the benchmark paper.** `"gap"` is the default and is unchanged (verified by diffing against `git show HEAD:` across 576 parameter/data combinations, 0 mismatches; `.find_consensus_by_agreement()` delegates to the existing `.find_lca()` whenever `agreement_fraction >= 1`, so unanimity is the same code it always was). `"bracket"` implements Jonah Ventures' actual published rule: a 1% bracket anchored at the ESV's top score, a taxon reported at a rank only when it holds >= 90% of the HITS in that bracket, NA otherwise, and a widen-to-2% fallback when a 97%-or-better match still returns no family. **The old roxygen's claim that the gap-style algorithm reproduced "the GITA / Jonah Ventures convention" was false and is removed** -- gap mode is the generic fixed-threshold/LCA convention. This is not cosmetic: strict unanimity upranks more readily than a 90% rule, so a TaxaID-vs-`score_consensus()` comparison run in gap mode is biased toward TaxaID on resolution (same-name rate vs JV's delivered taxonomy: 0.9999 at 0.9, but 0.9957/0.9918 at 1.0 -- unanimity disagrees ~60x more often). `ecosystem_docs/AQUARIUM_BENCHMARK_DESIGN.md` and `TODO_validation_benchmark.md`, which both proposed gap mode AS the JV benchmark, were corrected too. Validated at **14,718/14,719 ESVs (0.9999)** against JV's own delivered Pt Conception MiFish consensus taxonomy -- `diagnostics/jv_bracket_consensus_validation.R`, run on JV's own detailed-hit tables so the reference library is held constant and any disagreement is the rule, not the database. Three JV ambiguities decided and flagged (bracket lower bound implemented half-open per spec though JV's real bound is closed -- costs 1 ESV; "no family level taxonomy" read as coarser-than-family-or-absent; anchored bracket rather than a stepping loop, confirmed empirically). See `TaxaAssign/CLAUDE.md`'s top session note for the full record. 13 new tests, `devtools::check()` 0/0/1 (pre-existing NOTE). |
| 2026-09-14 (Opus 5) | `TaxaTools::cache_ok()` + `taxaid_cache_report()` added; `list_cache_files(recursive = FALSE)` added | TaxaTools | **Additive, fully backward compatible** (cache policy P4/P6). `cache_ok(path, inputs = NULL)` is the ecosystem's staleness primitive, lifted VERBATIM from three byte-identical copies pasted into `PtConceptionWorkflow_12S_single_site.R`/`_18S_2_single_site.R`/`GreatLakes2023_ConsensusWorkflow.R` -- those three now read `.cache_ok <- TaxaTools::cache_ok`, behaviour unchanged. `taxaid_cache_report(extra_dirs, warn_gb)` sums all six package caches plus any project-local dirs, recursively, and **reports only, never deletes** -- the caches do not share one shape, and TaxaMatch's is row-level/TTL'd, so file-granularity deletion there would discard live rows. `list_cache_files()`'s new `recursive` defaults `FALSE`, preserving every existing caller; it also now drops directories, which the old code would have handed to `file.remove()` if one ever matched a pattern. |
...
| 2026-09-15 (Opus 5) | `estimate_kernel_priors()` emits `prior_branch = "kernel_estimated"` (was `"resident_observed"`) | TaxaExpect | **Rename only -- no row moves branch, no number changes.** The old name asserted an evidence claim the estimator never tests: it writes the label as a CONSTANT, while `effective_records` within that one branch spans ~10 orders of magnitude (44.9% of real PtCon 12S rows under one effective record, minimum 0.0000; 64.9% at 18S; 18.9% at GreatLakes). **The old string is still accepted by every reader in this ecosystem, permanently** -- prior tables checkpointed before today carry it across four sites, and recognising only the new name would silently reclassify all of them. If you have your own code filtering on `prior_branch`, change `== "resident_observed"` to `%in% c("kernel_estimated", "resident_observed")`; a bare equality will match zero rows the next time priors are regenerated while looking fine on every cached table. All 17 such filters in the 6 production workflows were patched. |
| 2026-09-15 (Opus 5) | `posterior_consensus(min_effective_records = 0)` added | TaxaAssign | **Additive, default is the previous behaviour exactly.** Gates `winner_has_occurrence_record` on the winning row's `effective_records`, because branch membership alone cannot carry an evidence claim. `0` means branch-only. Raising it moves rows between Axis-1 plausibility categories and therefore changes published counts -- re-run the held-out GreatLakes/Lamar check (benchmark precision 0.872) before trusting a non-zero value. No threshold is defaulted on because the distribution is continuous with no natural break; one Kish effective record is the defensible starting point. A legacy table with no `effective_records` column keeps its branch verdict rather than being demoted. |
| 2026-09-15 (Opus 5) | `compute_group_priors()` default `rank_cols` gains `"order"`, `"class"`; a DEFAULT rank absent from `taxonomy_map` is now skipped, not an error | TaxaAssign | **Behavioural once callers supply the columns; safe for existing callers.** A consensus resolving at a rank the map does not cover finds no group row, reads `consensus_has_occurrence_record = FALSE` and is reported `"unprecedented"` from the gap alone -- the documented Mugu 504/616 species-rank bug, now closed one rank coarser. Real cost: the PtConception `Perciformes` unit (20 observations) was auto-flagged unprecedented, the review's skepticism gate then rated it geographically unlikely against its own comment calling the candidates "very common at Pt. Conception", and the export dropped all 20. A rank named EXPLICITLY and absent is still an error; only defaults adapt, so a `taxonomy_map` stopping at family behaves as before with a message. **The 5 production call sites pass `rank_cols` explicitly and their `taxonomy_map` carries no order/class, so they are UNCHANGED until wired** -- doing so changes plausibility counts and wants the Lamar re-check. |
