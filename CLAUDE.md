...
# Last updated: 2026-06-24 (Session 119 — score_image_inat() added to TaxaMatch; live iNat CV API submission with EXIF, path metadata, n_observations)
...
| 121 | Per-species sigma floor in `evaluate_likelihoods()` | TaxaLikely | `use_sigma[1,1]` is now floored at `global_sigma[1,1]` in `.evaluate_one_query()` before H1 density evaluation. Prevents artificially tight per-species distributions (from near-identical NCBI reference clones) from driving H1 likelihood to zero for realistic eDNA query scores. Worst case without fix: legitimate species silently dropped below `ratio_threshold`. No API or interface change — inference behaviour improvement only. |
