...
| 104 | `flag_prior_mismatch()` added | TaxaFlag | Flags assignments where winner is prior-driven (low likelihood) or unexpected (low prior + adequate likelihood); adds `prior_mismatch_risk`/`_score`/`_reason`; uses `winner_prior`/`winner_likelihood` from `posterior_consensus()` |
