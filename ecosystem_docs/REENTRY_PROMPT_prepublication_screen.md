...
| Branch `gbif-facet-priors` (`5f5ae9d`, "count based priors" chat) | THREE NEW EXPORTS (`TaxaFetch::fetch_gbif_occurrence_counts()`, `TaxaFetch::plan_gbif_fetch()`, `TaxaExpect::estimate_kernel_priors_from_counts()`), one new arg (`estimate_kernel_priors(count_col =)`, NULL keeps behaviour), one RENAME (`fetch_inat_occurrences()` -> `fetch_inat_occurrence_counts()`, separate commit `9a05644`, no caller outside TaxaFetch/TaxaExpect); reviewed detached against the branch's own library: TaxaFetch 866/0, TaxaExpect 763/0, check 0/0/0 both, READMEs complete, review-response rows in; `ecosystem_docs/HANDOFF_counts_path_prior.md` on the branch belongs in TaxaID_dev; install order TaxaFetch before TaxaExpect | maintainer: the three exports, the rename (A3 said no renames), and the merge |
| Bimodality warning (`calibrate_query_noise()` / `.bimodality_check()`) | proposed by the optimize-matrix chat, NOT built: the COI "H1 scores look bimodal" warning is a point mass at 100% identity from duplicate conspecific submissions (59% of H1 pairs, sd 0.018) plus the real continuum; structural to any reference set, the advice to calibrate platforms separately cannot resolve it, and `Stats$h1_bimodality$explanation` is NA where it should say so. Fix: name the point-mass reading in `explanation` and soften the warning; inside existing functions, no export | maintainer's go, then review here |
| Workflow checks after the reinstall | stale-argument check on the six edited workflows: only `pair_retention`/`min_pair_coverage` stale, as expected until the install; `check_dead_calls.sh` needs a two-root fix (archives now in TaxaID_dev, NAMESPACE in TaxaID) | the reinstall; the script fix is the screen session's |
| CalIntertidal workflow | its retention lines sit uncommitted in that chat's working tree with ~920 lines of its own Section 7 work; that chat commits its own file; `MAX_SEQS_PER_GENUS` untouched | maintainer: drop the genus cap now or after the memory branch; how the cheap 12S retention re-run is executed |
