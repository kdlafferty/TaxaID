...
| PtCon 12S cheap re-run under retention | approved by the maintainer; the optimize-matrix chat runs it via Rscript after the reinstall, with notice, backing up the Step 7 checkpoints it rewrites | the reinstall |
| CalIntertidal workflow | its retention lines sit uncommitted in that chat's working tree with ~920 lines of its own Section 7 work; that chat commits its own file; `MAX_SEQS_PER_GENUS` set to NULL there by the maintainer's decision (uncommitted with that chat's Section 7 work; that chat commits) | that chat's commit |
