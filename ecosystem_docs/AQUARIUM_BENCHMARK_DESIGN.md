...
| 1 | Consensus-LCA (Jonah Ventures rule) | DADA2+BLAST pipelines | `score_consensus(consensus_mode = "bracket", agreement_fraction = 0.9, bracket_width = 1, bracket_fallback = list(min_score = 97, rank = "family", width = 2))` — **corrected 2026-09-12**: the old entry here (`score_consensus(max_gap = 1, rank_thresholds = ...)`) was NOT the Jonah Ventures rule. Their rule reports a taxon at a rank when it holds ≥90% of the hits in the bracket; gap mode requires strict unanimity, which upranks more readily and so biases any TaxaID-vs-conventional comparison toward TaxaID on resolution. Validated against JV's own delivered Pt Conception MiFish taxonomy at 14,718/14,719 ESVs — see `diagnostics/jv_bracket_consensus_validation.R`. Where a provider ships its own consensus taxonomy, benchmark against that directly rather than against a reconstruction. |
