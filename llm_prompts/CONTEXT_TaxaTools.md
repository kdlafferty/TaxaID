...
| Param | Required | Default | Doc |
|---|---|---|---|
| ... | yes |  | 'report_section' objects, in any order. title: Character or 'NULL'. Optional report title. |
| title | no | NULL |  |
...
| cache_dir | no | NULL | Optional directory for a persistent, content-keyed cache of the harmonisation lookup (one '.rds' file, keyed on the sorted unique rank names actually resolved) - only used, and only relevant, when 'harmonise = TRUE'. 'NULL' (default) does no caching, so every call re-resolves every unique name. verbose: Logical, default 'TRUE'. Print one table of the resulting 'sampling_group' counts (including the 'NA' count) after classification, and progress messages during harmonisation. |
| verbose | no | TRUE |  |
...
| citations | no | NULL | Character vector. References to incorporate into the text (e.g., 'c("Callahan et al. (2016) DADA2", "R Core Team (2025)")'). Default 'NULL'. facts: Named list. Domain-specific grounding facts as key-value pairs. Use this for any information that does not fit the other fields (e.g., 'list(location = "Palmyra Atoll", marker = "12S MiFish", sampling_year = 2017, model_type = "VAR(2)")'). Default 'NULL'. |
| facts | no | NULL |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| path | yes |  | Character. Path to the candidate cache file. inputs: Character vector or 'NULL'. Paths this cache was derived FROM. Missing and 'NA' entries are ignored, so a caller can pass an optional upstream without branching. If any surviving input is newer than 'path', the cache is rejected. |
| inputs | no | NULL |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt_str | yes |  | Character. A length-1 string containing the complete prompt to submit. model: Character. Exact Anthropic model identifier, e.g. '"claude-sonnet-4-6"'. Default 'NULL' resolves to the latest model for 'tier' via 'list_models'. Specify an exact model to pin a version for reproducibility. tier: Character. Capability tier used when 'model = NULL': '"fast"' (cheapest), '"mid"' (balanced, default), or '"top"' (most capable). Ignored when 'model' is specified. |
| model | no | NULL |  |
| tier | no | c("mid", "fast", "top") |  |
| max_tokens | no | 3000L | Integer. Maximum tokens in the response (default 3000). Sufficient for most taxonomy and habitat prompts. Increase for longer outputs (e.g., large species lists). Higher values increase API cost. api_key: Character. Anthropic API key. Defaults to the 'ANTHROPIC_API_KEY' environment variable. |
| api_key | no | Sys.getenv("ANTHROPIC_API_KEY") |  |
...
| provider | no | NULL | Character. Provider name: '"anthropic"', '"gemini"', '"openai"', '"azure_openai"', '"ollama"', or any name registered with 'register_provider'. Default 'NULL' uses 'options("TaxaID.provider")', which is set automatically by 'library(TaxaTools)' based on detected API keys. tier: Character. Capability tier when 'model = NULL': '"fast"' (cheapest/smallest), '"mid"' (balanced, default), or '"top"' (most capable). Ignored when 'model' is specified. Tier-to-model mapping is discovered live from the provider's '/models' endpoint (see 'list_models'). model: Character. Exact model identifier. Overrides 'tier' resolution. Use to pin a specific version for reproducibility. |
| tier | no | c("mid", "fast", "top") |  |
| model | no | NULL |  |
| max_tokens | no | 3000L | Integer. Maximum tokens in the response (default 3000). The correct request body field for the provider ('max_tokens' vs 'max_completion_tokens') is read from the registry automatically. api_key: Character. API key override. Default 'NULL' reads the key from the environment variable named in the provider's registry entry (e.g. 'ANTHROPIC_API_KEY'). Keyless providers (Ollama) ignore this. |
| api_key | no | NULL |  |
| base_url | no | NULL | Character. Base URL override for OpenAI-compatible providers (OpenAI, Azure, Ollama, custom registered providers). Default 'NULL' uses the provider's registered endpoint. • For Ollama: change the host/port, e.g. 'base_url = "http://remote-server:11434"'. • For OpenAI-compatible proxies: supply the proxy base URL. • For Azure: replaces the host in the endpoint template while preserving the deployment path and API version. images: Named list of base64-encoded PNG strings, as returned by '.render_pdf_pages()' in the TaxaFetch PDF pipeline. Default 'NULL' (text-only call). When supplied, the prompt and images are sent as a multi-modal message using the provider's vision format: Anthropic image content blocks, Gemini 'inlineData' parts, or OpenAI 'image_url' blocks. Requires a vision-capable model (e.g. Claude Sonnet, Gemini 2.5 Flash, GPT-4o). |
| images | no | NULL |  |
...
| max_input_tokens | no | NULL | Integer or 'NULL'. When non-'NULL', estimates the prompt length as 'ceiling(nchar(prompt_str) / 3.5)' (a conservative characters-per-token heuristic) and stops with an informative error before making the HTTP request if the estimate exceeds the limit. Use this as a pre-flight guard against accidentally sending very large prompts. Default 'NULL' (no check performed). timeout: Numeric. Request timeout in seconds, passed to 'httr2::req_timeout()'. Default '120'. Raise this for a slow provider/model (e.g. a large local Ollama model) or a large multi-image PDF-vision call that can legitimately take longer than two minutes. |
| timeout | no | 120 |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt_str | yes |  | Character. A length-1 string containing the complete prompt to submit. model: Character. Azure deployment name, e.g. '"gpt-5.1"'. Default 'NULL' resolves to the latest available deployment for 'tier' via 'list_models'. Specify an exact name to pin for reproducibility. tier: Character. Capability tier used when 'model = NULL': '"fast"', '"mid"' (default), or '"top"'. For Azure, all tiers currently map to the same DOI deployment; the param is accepted for interface consistency. |
| model | no | NULL |  |
| tier | no | c("mid", "fast", "top") |  |
...
| max_completion_tokens | no | 3000L | Integer. Maximum tokens in the response (default 3000). Azure o-series models use this field name instead of 'max_tokens'; handled automatically via the registry. api_key: Character. Azure OpenAI API key. Defaults to the 'AZURE_OPENAI_API_KEY' environment variable. |
| api_key | no | Sys.getenv("AZURE_OPENAI_API_KEY") |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt_str | yes |  | Character. A length-1 string containing the complete prompt to submit. model: Character. Exact Gemini model identifier, e.g. '"gemini-2.5-flash"'. Default 'NULL' resolves to the latest model for 'tier' via 'list_models'. Specify an exact model to pin a version for reproducibility. tier: Character. Capability tier used when 'model = NULL': '"fast"' (cheapest), '"mid"' (balanced, default), or '"top"' (most capable). Ignored when 'model' is specified. |
| model | no | NULL |  |
| tier | no | c("mid", "fast", "top") |  |
| max_tokens | no | 3000L | Integer. Maximum tokens in the response (default 3000). Sufficient for most taxonomy and habitat prompts. Increase for longer outputs (e.g., large species lists). Higher values increase API cost. api_key: Character. Google AI Studio API key. Defaults to the 'GEMINI_API_KEY' environment variable. Get a free key at <https://aistudio.google.com/apikey> and add 'GEMINI_API_KEY=AIza...' to '~/.Renviron'. |
| api_key | no | Sys.getenv("GEMINI_API_KEY") |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt_str | yes |  | Character. A length-1 string containing the complete prompt to submit. model: Character. Ollama model name as listed by 'ollama list' in Terminal. Default '"llama3.2"'. Pull a model before first use: 'ollama pull llama3.2'. Browse available models at <https://ollama.com/library>. Capable options for Apple Silicon: '"llama3.1:8b"', '"mistral"', '"gemma3:12b"', '"qwen2.5:14b"'. |
| model | no | "llama3.2" |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt_str | yes |  | Character. A length-1 string containing the complete prompt to submit. model: Character. Exact OpenAI model identifier, e.g. '"gpt-4o-mini"'. Default 'NULL' resolves to the latest model for 'tier' via 'list_models'. Specify an exact model to pin a version for reproducibility. tier: Character. Capability tier used when 'model = NULL': '"fast"' (cheapest), '"mid"' (balanced, default), or '"top"' (most capable). Ignored when 'model' is specified. |
| model | no | NULL |  |
| tier | no | c("mid", "fast", "top") |  |
...
| base_url | no | "https://api.openai.com" | Character. Base URL of the API endpoint. Default '"https://api.openai.com"' (OpenAI). Any OpenAI-compatible API can be used by changing this URL - see Details. When using a non-default URL, register the provider with 'register_provider' to enable automatic tier resolution, or specify 'model' explicitly. api_key: Character. API key for the provider. Defaults to the 'OPENAI_API_KEY' environment variable for the standard OpenAI endpoint. For alternative providers, pass the key directly or via 'Sys.getenv("MY_KEY_VAR")' in a closure. |
| api_key | no | Sys.getenv("OPENAI_API_KEY") |  |
...
| match_species | no | NULL | Character vector of species already in the reference database (e.g., 'unique(match_df$species)'). If provided, the function computes per-genus completeness: how many described species are missing from the reference. If NULL (default), only the GBIF census is returned without reference comparison. rank: Character string indicating the rank of the input keys. Default '"genus"'. For higher ranks (e.g., '"family"', '"order"'), the function recursively enumerates child genera, then species within each genus. |
| rank | no | "genus" |  |
| status_filter | no | "ACCEPTED" | Character vector of GBIF 'taxonomicStatus' values to include. Default '"ACCEPTED"'. Set to 'c("ACCEPTED", "DOUBTFUL")' to include taxonomically uncertain species. verbose: Logical. If TRUE (default), prints progress messages. |
| verbose | no | TRUE |  |
...
| backbone_id | no | 11L | Integer. Taxonomic backbone for verification. '11' = GBIF (default), '3' = ITIS, '4' = NCBI, '9' = WoRMS, '1' = Catalogue of Life. verify: Logical. If 'TRUE' (default), runs 'verify_taxon_names' on each non-'NA' LLM suggestion and populates 'scientific_name_verified' and 'verified'. llm_fn: Function with signature 'function(prompt, ...) -> character(1)'. Default 'getOption("TaxaID.llm_fn")'. ...: Additional arguments passed to 'llm_fn'. |
| verify | no | TRUE |  |
| llm_fn | no | getOption("TaxaID.llm_fn") |  |
| ... | yes |  |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| lat | no | NULL | Numeric. Latitude of the centre point in decimal degrees (WGS 84). Ignored when 'init_polygon' is supplied (the centre is derived from the polygon instead). Required otherwise. lon: Numeric. Longitude of the centre point in decimal degrees (WGS 84). Ignored when 'init_polygon' is supplied. Required otherwise. |
| lon | no | NULL |  |
| radius_deg | no | NULL | Numeric. Half-width of the initial square in decimal degrees. For reference: 1 degree ~= 111 km. Ignored when 'init_polygon' is supplied. Required otherwise. tile: Character. Leaflet tile provider name. Default '"Esri.OceanBasemap"'. Any string accepted by 'leaflet::addProviderTiles()' works (e.g. '"OpenStreetMap"', '"Esri.WorldImagery"'). points: Data frame of observation coordinates to overlay on the map as small, non-interactive reference markers (does not affect the returned polygon). Must have 'lat' and 'lng' columns. Useful for seeing the actual data cloud while drawing a group or search-area boundary around it. Default 'NULL' (no overlay). |
...
| rank_system | no | NULL | Character vector of rank names (coarse to fine), or 'NULL' to auto-detect from 'standard_ranks'. warn: Logical (default 'TRUE'). If 'TRUE', emits a warning when auto-detection finds no standard rank columns and falls back to the minimum set. |
| warn | no | TRUE |  |
...
| description | no | NULL | Character. Brief study context to guide the LLM (e.g., '"eDNA metabarcoding of coral reef fish at Palmyra Atoll using MiFish 12S primers"'). Default 'NULL'. Ignored when 'context' is provided (uses 'context$study_description'). context: A 'report_context' object from 'build_report_context'. Provides structured facts (study description, parameters, statistics, citations) that the LLM must use as ground truth. When provided, overrides 'description'. Default 'NULL'. |
| context | no | NULL |  |
| audience | no | "journal" | Character. Target audience: '"journal"' (default) for peer-reviewed publication style, '"technical"' for a methods appendix with parameter details, or '"brief"' for a short summary. llm_fn: Function. LLM provider function with signature 'function(prompt_str, ...) -> character(1)'. Default 'getOption("TaxaID.llm_fn", call_api)' - the ecosystem-wide auto-detected provider (see 'call_api'), falling back to 'call_api' itself if the option is unset. |
| llm_fn | no | getOption("TaxaID.llm_fn", call_api) |  |
| max_code_lines | no | 300L | Integer. Maximum number of code lines to include in the prompt. Long scripts are truncated with a note. Default '300L'. verbose: Logical. Print progress messages. Default 'FALSE'. |
| verbose | no | FALSE |  |
...
| description | no | NULL | Character. Brief study context. Default 'NULL'. Ignored when 'context' is provided. context: A 'report_context' object from 'build_report_context'. Provides structured facts the LLM must use as ground truth. Default 'NULL'. |
| context | no | NULL |  |
| audience | no | "journal" | Character. Target audience: '"journal"' (default), '"technical"', or '"brief"'. code: Character. Optional R code (file path or lines) that produced the objects. When provided, the LLM can reference specific analysis steps. Default 'NULL'. llm_fn: Function. LLM provider function. Default 'getOption("TaxaID.llm_fn", call_api)' - the ecosystem-wide auto-detected provider (see 'call_api'), falling back to 'call_api' itself if the option is unset. |
| code | no | NULL |  |
| llm_fn | no | getOption("TaxaID.llm_fn", call_api) |  |
| max_rows | no | 20L | Integer. Maximum rows to show per data frame in the prompt. Default '20L'. verbose: Logical. Print progress messages. Default 'FALSE'. |
| verbose | no | FALSE |  |
...
| fallback_backbone_id | no | 11L | Integer. Secondary backbone tried if the primary backbone doesn't resolve 'taxon_name'. Default '11L' (GBIF). Set 'NULL' or equal to 'backbone_id' to skip. verbose: Logical. Print progress messages. Default 'TRUE'. |
| verbose | no | TRUE |  |
...
| batch_size | no | 50L | Integer, default 50. Names per name-match request. Requests are additionally split to keep the URL under ~6,000 characters, so a batch of long names is chunked further on its own. delay: Numeric seconds between requests, default 0.5. Applies between name batches and between per-taxon extra calls. verbose: Logical, default 'TRUE'. Progress and a summary. |
| delay | no | 0.5 |  |
| verbose | no | TRUE |  |
...
| fallback_backbone_id | no | 11L | Integer. Secondary backbone used when 'backbone_id' returns no match. Default '11L' (GBIF). Set 'NULL' or equal to 'backbone_id' to skip. verbose: Logical. Print progress messages for API lookups. Default 'TRUE'. |
| verbose | no | TRUE |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| package | yes |  | Character. Package name (e.g. '"TaxaFetch"'). section: Character. Short section identifier (e.g. '"fetch"', '"match"', '"likelihood"'). methods: Character. Methods text (template-based, deterministic). results: Character. Results text (template or LLM-generated). |
| section | yes |  |  |
| methods | yes |  |  |
| results | no | NULL |  |
| citations | no | NULL | Character vector or 'NULL'. Bibliographic citations associated with this pipeline step. params: Named list or 'NULL'. Key parameters used in this step (for reproducibility and downstream assembly). |
| params | no | NULL |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| path | yes |  | Character scalar. Pipe-delimited taxon names, e.g. '"Animalia\|Chordata\|Cottidae\|Cottus"'. ranks: Character scalar. Pipe-delimited rank labels aligned with 'path', e.g. '"kingdom\|phylum\|family\|genus"'. |
| ranks | yes |  |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt | yes |  | An 'llm_prompt' object (e.g. from 'build_habitat_prompt()' (TaxaHabitat)). llm_fn: Function. A provider function that accepts a single character string (the prompt) and returns a single character string (the response). Default: 'call_api' (uses 'options("TaxaID.provider")'). Built-in alternatives: 'call_anthropic_api', 'call_gemini_api', 'call_openai_api', 'call_ollama_api'. To use a non-default model or API key, pass a closure: my_gemini <- function(p, ...) call_gemini_api(p, model = "gemini-2.5-flash-lite") raw_text <- prompt_api(prompt, llm_fn = my_gemini) |
| llm_fn | no | getOption("TaxaID.llm_fn", call_api) |  |
| pause_seconds | no | 1 | Numeric. Seconds to pause between chunks to avoid rate limits. Default 1. verbose: Logical. Print progress per chunk. Default 'TRUE'. |
| verbose | no | TRUE |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| prompt | yes |  | An 'llm_prompt' object (e.g. from 'build_habitat_prompt()' (TaxaHabitat)). out_dir: Character. Directory to write prompt and response files. Default 'getwd()'. Created if it does not exist. prefix: Character. Filename prefix. Default '"habitat"'. Files are named '<prefix>_prompt_1.txt', '<prefix>_prompt_2.txt', etc. |
| out_dir | no | getwd() |  |
| prefix | no | "habitat" |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| input_df | yes |  | A data frame whose columns are to be renamed. col_map: Named character vector or 'NULL'. When supplied, each *name* is an existing column name in 'input_df' and each *value* is the desired new name. Both must be *quoted strings*: col_map = c("Latitude" = "decimalLatitude", "Longitude" = "decimalLongitude", "SurveyDate" = "eventDate") When 'col_map' is supplied it *replaces* the default pattern matching entirely — only the mappings you specify are applied. When 'NULL' (default), the built-in regex patterns are used instead (see Details). strict: Logical. Controls behaviour when a 'col_map' key is not found in 'input_df'. • 'FALSE' (default): warns about unmatched keys and renames whatever it can. Suitable when 'rename_cols()' is applied to frames that may only contain some of the target columns. • 'TRUE': stops with an error if any 'col_map' key is absent from 'input_df'. Use in scripts where all mappings must succeed. Has no effect when 'col_map = NULL' (default patterns always skip non-matching columns silently). |
| col_map | no | NULL |  |
| strict | no | FALSE |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| inv | yes |  | A data frame with 'path', 'size_mb', 'mtime' columns, as returned by 'list_cache_files()'. label: Character. Name shown in messages - typically the calling '<pkg>_clear_cache()' function's own name, so a user sees which function actually printed a given message. |
| label | yes |  |  |
| cache_dir | yes |  | Character. Shown in messages only; 'inv' is used as-is and is not re-scanned. |
| older_than_days | no | NULL | Numeric or 'NULL'. When supplied, only rows of 'inv' older than this many days (by 'mtime') are targeted. 'NULL' (default) targets every row of 'inv'. dry_run: Logical. If 'TRUE', reports what would be removed without removing anything. Default 'FALSE'. |
| dry_run | no | FALSE |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| provider | yes |  | Character. Provider name: '"anthropic"', '"gemini"', '"openai"', or '"azure_openai"'. tier: Character. Tier to pin: '"fast"', '"mid"', or '"top"'. model: Character. Exact model identifier to use, or 'NULL' to remove an existing pin. |
| tier | yes |  |  |
| model | yes |  |  |
...
| Param | Required | Default | Doc |
|---|---|---|---|
| extra_dirs | no | NULL | Character vector of additional cache directories to include - typically the project-local ones a workflow passes as 'cache_dir'. Non-existent paths are reported as missing rather than dropped, so a typo is visible. warn_gb: Numeric. Directories at or above this size are flagged in the printed output. Default 1. Set 'NULL' to disable flagging. |
| warn_gb | no | 1 |  |
...
| older_than_days | no | NULL | Numeric or 'NULL'. Only files older than this many days are targeted; 'NULL' (default) targets every file. dry_run: Logical. If 'TRUE', reports without deleting. |
| dry_run | no | FALSE |  |
