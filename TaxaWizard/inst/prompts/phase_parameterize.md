...
- `taxon_col` / `taxon_rank_col` for `review_assignments()` and `flag_contaminant()`: when data comes from TaxaAssign (`score_consensus()`, `posterior_consensus()`, `run_llm_pipeline()`), use `"consensus_taxon"` and `"consensus_rank"`. For external/user-supplied data, ask the user what column names their data actually uses — do NOT assume `"consensus_taxon"`. Similarly, `event_col` defaults to `"observation_id"` for TaxaAssign output but must be confirmed for external data.
...
- `rank_system` for acoustic/image: For BirdNET acoustic data, `rank_system = c("family", "genus", "species")` is typical (family is needed for `unreferenced_candidates()` H3 rows). For image classifiers, use whatever taxonomy columns the classifier returns.
...
- `search_radius_deg`: Default 5. Sets how far GBIF occurrence records are fetched around the site. Too small leaves the kernel prior with too few records to estimate composition; too large dilutes local signal. Recommend 5-6 for most use cases.
