...
#' The 8 installable TaxaID packages, in dependency order
#'
#' Mirrors the \code{TAXAID_PACKAGES} constant the ecosystem's introspected
#' registry (\code{R/registry.R}, a sibling work package) also defines. Kept
#' as a private function rather than a bare package-level object so the two
#' cannot collide if both land in the same namespace: the values are
#' identical, but this one is named differently on purpose.
#'
#' @return Character vector.
#' @noRd
.taxaid_packages <- function() {
  c(
